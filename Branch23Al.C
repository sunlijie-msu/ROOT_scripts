...
void Branch23Al()
{
	...
	while (iT<=7)
	{
		...
		{
			sprintf(anarootname,"%s%04d%s%04d%s","V:/RIBLL2015/data24/Al23calabcd_",ianaroot,"_",ianaroot,"p10T.root");//modify
			...
			//cin>>D_name;
			sprintf(D_name,"%s","D60Bne");//modify
			//cout<<D_name;
			...
			sprintf(filename,"%s%s%s","C:/Si24/Si22peakcali/Branch",D_name,"_peakch23Al.dat");//modify
			...
			for(int ipeak=0;ipeak<4;ipeak++)
			{
				sprintf(D_command,"%s%s%f%s%s%s%f%s%d",D_name,">",chlow[ipeak],"&&",D_name,"<",chhigh[ipeak],"&&TAB60<",450*iT);//modify
				puts(D_command);
