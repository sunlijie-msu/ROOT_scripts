...
const int Binsperkev=10; //Number of bins per keV, the only place to change binwidth, all other variables is related to Binsperkev //modify 10 for NSCL, 2 for RIBLL
...
const double E0_gamma=1366; //E0+i*binwidth modify, a baseline E0 determined by the file name of the simuroot.root, 2752 4236 2868 1366, decide which simuroot to use// if you want to use for() loop and E0_gamma is related to iscan, E0_gamma cannot be global variable out of the main(), as loop has to be in the main().
const double Ebin_gamma=1369; //modify center for drawing figures: 2754 4238 2870 1369, don't change to other values // if you want to use for() loop and E0_gamma is related to iscan, E0_gamma cannot be global variable out of the main(), as loop has to be in the main().
...
const double minrangezoom=Ebin_gamma-25; //shorter range to calculate chi2 25 30 30 40 modify
const double maxrangezoom=Ebin_gamma+25; //shorter range to calculate chi2 25 30 30 40
...
double T_Lifetime = 1918.78;//31.74;59.15;1918.78                   // This is Excited State Mean Lifetime (fs), Mean Lifetime τ=T/ln(2); half-life T=τ*ln(2) modify
//double E0_gamma=2753.0+double(iscan)*2.0/10.0;                  // Energy of Gamma-ray (keV) useless
const int nBranches=20;                  //13,19;2,3 Number of proton Branches feeding daughter modify
TH1F *proton_branch[nBranches],*proton_copy[nBranches],*maximum_branch[nBranches],*totalmax;
//Lit 1369-keV γ 19
double E_CoM[nBranches] = {4.551,	4.345,	4.257,	3.606,	3.464,	3.236,	3.006,	2.486,	2.453,	2.164,	1.268,	0.944,	0.554,	1.794,	1.492,	1.037,	1.684,	1.380,	0.724,	0.000};         //Center of Mass energy (MeV) in descending order
double feed[nBranches]={0.013255,	0.020440,	0.465222,	0.046075,	0.163569,	0.019126,	0.018993,	0.004343,	0.001810,	0.078835,	0.018351,	0.077865,	0.022339,	0.023108235,	0.011642475,	0.007423944,	0.002110709,	0.004468022,	0.000405499,	0.000618};		//beta feeding intensity of each proton, if the sum of all the feeding intensities is less than 1, the extra events will assume a En=0.
//Lit 2754-keV γ 4
// double E_CoM[nBranches] = {1.805,1.501,1.040,0.000};         //Center of Mass energy (MeV) in descending order
// double feed[nBranches]={6.73/(6.73+2.90+1.53+0.522),2.90/(6.73+2.90+1.53+0.522),1.53/(6.73+2.90+1.53+0.522),0.522/(6.73+2.90+1.53+0.522)};		//beta feeding intensity of each proton, if the sum of all the feeding intensities is less than 1, the extra events will assume a En=0.
//Lit 2870-keV γ 3
...
void Comparison_1369_gated_Lit()
{
	for(int iscan=34;iscan<35;iscan++){//portal, scan many simulation rootfile with different gamma energies 1369 i=34; 2754 i=29; 2870 i=30; 4238 i=27
		...
		if(Ebin_gamma==1369)	{fitrangelow1=minrangeb+30; fitrangelow2=minrangeb+80; fitrangehigh1=maxrangeb-80; fitrangehigh2=maxrangeb-20;}
		...
		//sprintf(tname,"%s%.2f%s%d%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Thomas/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Eg",E0_gamma,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",0.1,"_P10_Sun_Thomas");
		//Lit
		sprintf(tname,"%s%.2f%s%d%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Lit/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Eg",E0_gamma,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",0.1,"_P10_Sun_Lit_baseline_August");
		...
		//	sprintf(simurootname,"%s%.2f%s%d%s%.1f%s%d%s%s%s%d%s%d%s%d%s%.1f%s","D:/X/out/Lit/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Eg",E0_gamma,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_binwid",0.1,"_Si_Sun_Lit.root");
		fin_simu = new TFile(simurootname); //Rootfile with Simulation histograms
		//	fin_data = new TFile("D:/X/out/Si25/100eV-bin-ungated_sun_25Si.root"); //Rootfile with Experimental Data Histogram ungated modify
		fin_data = new TFile("D:/X/out/Si25/100eV-bin-gated_sun_25Si.root"); //Rootfile with Experimental Data Histogram gated
		//fin_data = new TFile("D:/X/out/Si25/100eV-bin-gated_sun_25Si_10peakscali.root"); //Rootfile with Experimental Data Histogram gated //bkgcali
		...
		{
			int bkgpower=7;//change this to whatever you want. 3 for high statistics
			...
			// 		else b = (a*pow(f,bkgpower)+c*pow(g,bkgpower))/h;
			b = (a*pow(f,bkgpower)+c*pow(g,bkgpower))/h;
		...
		for(int i=1;i<=Nbins;i++)
		{
			...
			{
				sprintf(hname,"p%df2",int(ii));
				proton_copy[ii]->SetBinContent(i,proton_branch[ii]->GetBinContent((Ebin_gamma-E0_gamma)*Binsperkev-iscan+i));//part of the simulated proton-γ histogram is stored in proton_copy
			}
		}
		...
		char paraprint[100];
		TPaveText *textchi = new TPaveText(0.10,0.87,0.38,0.97,"brNDC");//left, down, right, up
		...
		sprintf(paraprint,"-2lnL/ndf = %.2f / %d = %.5f",Chi2,(Nbinsz-2),Chi2/(Nbinsz-2));
		...
		hResid->GetXaxis()->SetNdivisions(520);//n = n1 + 100*n2 + 10000*n3
		// 	gr->GetXaxis()->SetNdivisions(10,10,1);
		hResid->GetYaxis()->SetNdivisions(505);
		hResid->GetYaxis()->SetLabelSize(0.06);
		hResid->GetXaxis()->SetLabelSize(0.06);
		...
		fprintf(outfile, "Emax=	%.2f	Emean=	%.1f	-2lnL =	%.3f	-2lnL/NDF =	%.5f	%d\n", centroid+iscan*binwidth,E0_gamma+iscan*binwidth,Chi2,Chi2/(Nbinsz-2),Nbinsz-2);//Chi2 is global varible, use Chi2 to extract the minimized Chi2 obtained in fcn(), and output Chi2 in main().

		//Thomas save figure modify
		//sprintf(tname,"%s%.2f%s%d%s%.1f%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Figure_gated/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Emax",centroid+iscan*binwidth,"_Emean",E0_gamma+iscan*binwidth,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",binwidth,"_P10_Sun_Thomas");
		//Lit save figure
		sprintf(tname,"%s%.2f%s%d%s%.1f%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Figure_gated/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Emax",centroid+iscan*binwidth,"_Emean",E0_gamma+iscan*binwidth,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",binwidth,"_P10_Sun_Lit");
		//Lit RIBLL save figure
		//	sprintf(tname,"%s%.2f%s%d%s%.1f%s%.1f%s%d%s%s%s%d%s%d%s%d%s%.1f%s","D:/X/out/Figure_gated_RIBLL/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Emax",centroid+iscan*binwidth,"_Emean",E0_gamma+iscan*binwidth,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_binwid",binwidth,"_Si_Sun_Lit");
