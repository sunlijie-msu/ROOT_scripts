...
const int nBranches=13;                  //13,19;2,3 Number of proton Branches feeding daughter modify
...
void Comparison_1369_ungated_Tho_rel()
{
	for(int iscan=34;iscan<35;iscan++){//portal, scan many simulation rootfile with different gamma energies 1369 i=37; 2754 i=35; 2870 i=34; 4238 i=36
		...
		sprintf(tname,"%s%.2f%s%d%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Thomas/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Eg",E0_gamma,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",0.1,"_P10_Sun_Thomas_baseline");
		//Robertson
		//sprintf(tname,"%s%.2f%s%d%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Robertson/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Eg",E0_gamma,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",0.1,"_P10_Sun_Robertson_baseline");
		...
		fin_data = new TFile("D:/X/out/Si25/100eV-bin-ungated_sun_25Si_jason0.5.root"); //Rootfile with Experimental Data Histogram ungated modify
...
//		hSega_data= (TH1F*)fin_data->Get("gbCoincidences"); //Get Gamma ray spectrum gated
		...
		TF1 *f3=new TF1("f2","gausn(0)+gausn(3)+pol1(6)",fitrangehigh1,fitrangehigh2);//range of right half of hist you want to fit the background to
		f3->SetParameters(1689.18,1377.64,1.595,423.899,1385.23,1.83885);
		f3->SetParNames("Const*bin1","Mean1","Sigma1","Const*bin2","Mean2","Sigma2");
		f2->SetParameters(3885.76,-2.50656);
		f2->SetParNames("BkgB","BkgA");
		hSega_data->Fit("f1","qR");
		//hSega_data->Fit("f2","qR");

		for(int i=1;i<=Nbinsb;i++)
		{
			int bkgpower=8;//change this to whatever you want. 3 for high statistics
			...
			double z = (f3->Eval(minrangeb+(double)i/(double)Binsperkev));//Upper BG
		...
		fprintf(outfile, "Emax=	%.2f	Emean=	%.1f	-2lnL =	%.3f	Chi2/NDF =	%.5f	%d\n", centroid+iscan*binwidth,E0_gamma+iscan*binwidth,Chi2,Chi2/(Nbinsz-2),Nbinsz-2);//Chi2 is global varible, use Chi2 to extract the minimized Chi2 obtained in fcn(), and output Chi2 in main().

		//Thomas save figure modify
		sprintf(tname,"%s%.2f%s%d%s%.1f%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Figure_ungated/Sim25Si_rel_T",T_Lifetime,"_Step",NumberofSteps,"_Emax",centroid+iscan*binwidth,"_Emean",E0_gamma+iscan*binwidth,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",binwidth,"_P10_Sun_Thomas");
		//Robertson save figure bkgcaliSim25Si_rel_T
		//sprintf(tname,"%s%.2f%s%d%s%.1f%s%.1f%s%d%s%s%s%d%s%d%s%d%s%d%s%d%s%d%s%.1f%s","D:/X/out/Figure_gated/Sim25Si_rel_T",T_Lifetime,"_Step",NumberofSteps,"_Emax",centroid+iscan*binwidth,"_Emean",E0_gamma+iscan*binwidth,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_st",Flag_st,"_sig",Flag_sig,"_tau",Flag_tau,"_binwid",binwidth,"_P10_Sun_Robertson");
		//Robertson RIBLL save figure
		//	sprintf(tname,"%s%.2f%s%d%s%.1f%s%.1f%s%d%s%s%s%d%s%d%s%d%s%.1f%s","D:/X/out/Figure_gated_RIBLL/Sim25Si_T",T_Lifetime,"_Step",NumberofSteps,"_Emax",centroid+iscan*binwidth,"_Emean",E0_gamma+iscan*binwidth,"_feed",nBranches,"_En",pname,"SP",Flag_SP,"_Det",Flag_Det,"_Ang",Flag_Ang,"_binwid",binwidth,"_Si_Sun_Robertson");
