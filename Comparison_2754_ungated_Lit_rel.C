...
const double E0_gamma=2752; //E0+i*binwidth modify, a baseline E0 determined by the file name of the simuroot.root, 2752 4236 2868 1366, decide which simuroot to use// if you want to use for() loop and E0_gamma is related to iscan, E0_gamma cannot be global variable out of the main(), as loop has to be in the main().
const double Ebin_gamma=2754; //modify center for drawing figures: 2754 4238 2870 1369, don't change to other values // if you want to use for() loop and E0_gamma is related to iscan, E0_gamma cannot be global variable out of the main(), as loop has to be in the main().
...
const double minrangezoom=Ebin_gamma-30; //shorter range to calculate chi2 25 30 30 40 modify
const double maxrangezoom=Ebin_gamma+30; //shorter range to calculate chi2 25 30 30 40
...
double T_Lifetime = 31.74;//31.74;59.15;1918.78                   // This is Excited State Mean Lifetime (fs), Mean Lifetime τ=T/ln(2); half-life T=τ*ln(2) modify
//double E0_gamma=2753.0+double(iscan)*2.0/10.0;                  // Energy of Gamma-ray (keV) useless
const int nBranches=4;                  //13,19;2,3 Number of proton Branches feeding daughter modify
...
//Robertson 2754-keV γ 4
   	double E_CoM[nBranches] = {1.794,	1.492,	1.037,	0.000};         //Center of Mass energy (MeV) in descending order
    double feed[nBranches]={0.5168427,	0.2603976,	0.1660452,	0.0567146};		//beta feeding intensity of each proton, if the sum of all the feeding intensities is less than 1, the extra events will assume a En=0.
...
void Comparison_2754_ungated_Lit_rel()
{
	for(int iscan=30;iscan<31;iscan++){//portal, scan many simulation rootfile with different gamma energies 1369 i=37; 2754 i=35; 2870 i=34; 4238 i=36
		...
		//fin_data = new TFile("D:/X/out/Si25/100eV-bin-ungated_sun_25Si_10peakscali.root"); //Rootfile with Experimental Data Histogram gated //bkgcali
