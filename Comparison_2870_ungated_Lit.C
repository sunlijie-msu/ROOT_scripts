...
const double E0_gamma=2868; //E0+i*binwidth modify, a baseline E0 determined by the file name of the simuroot.root, 2752 4236 2868 1366, decide which simuroot to use// if you want to use for() loop and E0_gamma is related to iscan, E0_gamma cannot be global variable out of the main(), as loop has to be in the main().
const double Ebin_gamma=2870; //modify center for drawing figures: 2754 4238 2870 1369, don't change to other values // if you want to use for() loop and E0_gamma is related to iscan, E0_gamma cannot be global variable out of the main(), as loop has to be in the main().
...
double E_CoM[nBranches] = {1.684,	1.380,	0.724,	0.000};         //Center of Mass energy (MeV) in descending order
double feed[nBranches]={0.2989817,	0.6328949,	0.0574389,	0.0106845};		//beta feeding intensity of each proton, if the sum of all the feeding intensities is less than 1, the extra events will assume a En=0.
//modify
...
void Comparison_2870_ungated_Lit()
{
