...
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)
	...
	for (flag_Eg = centroid - Eg_uncertainty*1; flag_Eg <= centroid + Eg_uncertainty*1; flag_Eg += Eg_uncertainty*1)
...
//	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
		...
		{
			for (flag_SP = 0.9; flag_SP <= 1.1; flag_SP += 0.1)
			{
...
// 			fclose(outfilehistMADAI);
			...
			sprintf(tname,"%s%.0f%s%04d","D:/X/out/DSL_MADAI_model_uncertainty/Eg",E0_gamma,"_G4_test_fewer_parameters/run",ifolder);
