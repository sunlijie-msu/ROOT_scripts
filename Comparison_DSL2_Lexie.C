...
//const int factor_rebin = 8; //simu and data Rebin factor
...
float bkgDown = 0.65;
float bkgUp = 0.85; // if you set bkgDown and bkgUp the same value, the bkg will be set to zero in minimization, which is wrong

const double E0_gamma = 2908; //Ex=7784.7, Eg=7333.2 by ENSDF
const int peakrange_min = 3040;
const int peakrange_max = 3094;
const double fitrange_min = 3030;
const double fitrange_max = 3098;
...
double Low_bkg = 0.81;
double High_bkg = 1.19;


 //double Tau_values[] = { 20.0 };
 //double Tau_values[] = { 1.0, 5.0, 10.0, 15.0 };
 //double Eg_values[] = { 2905.3, 2906.4, 2907.5 }; // 7331.20, 7333.20, 7335.20
 //double Bkg_values[] = { 1.00 }; // 0.90, 1.00, 1.10
 //double SP_values[] = { 1.00 }; // 0.90, 1.00, 1.10
 //double AD_values[] = { 0.0 };

 //double Tau_values[] = { 10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0 };
 //double Eg_values[] = { 7331.2, 7333.2, 7335.2 }; // 7331.20, 7333.20, 7335.20
 //double Bkg_values[] = { 0.90, 1.00, 1.10 }; // 0.90, 1.00, 1.10
 //double SP_values[] = { 0.90, 1.00, 1.10 }; // 0.90, 1.00, 1.10
 //double AD_values[] = { 0.0 };

double Tau_values[] = { 10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0, 20.0 };
double Eg_values[] = { 2906.4 };
	...
	for (int iEg = 0; iEg < sizeof(Eg_values) / sizeof(Eg_values[0]); iEg++)
	{
		for (int iBkg = 0; iBkg < sizeof(Bkg_values) / sizeof(Bkg_values[0]); iBkg++)
		{
			for (int iSP = 0; iSP < sizeof(SP_values) / sizeof(SP_values[0]); iSP++)
			{
				for (int iAD = 0; iAD < sizeof(AD_values) / sizeof(AD_values[0]); iAD++)
				{
					for (int iTau = 0; iTau < sizeof(Tau_values) / sizeof(Tau_values[0]); iTau++)
					{
						...
						//fin_data = new TFile("/mnt/analysis/triumf_s2193/lexie/Build_Geant4_DSL_Jan2025/RootFiles/7787Isotropic/Eg7333/groups2052all.root"); //Rootfile with Experimental Data Histogram modify
						//fin_data = new TFile("/mnt/analysis/triumf_s2193/lexie/treetest/nov25_7787triumf_oldbounds.root"); //Rootfile with Experimental Data Histogram modify
						fin_data = new TFile("D:/X/out/DSL2_Comparison/2908triumf.root"); //Rootfile with Experimental Data Histogram modify

						//Get Histograms from Simulation
						sprintf(hname, "group1to2");
						...
						sprintf(hname, "%s", "group1to2");//data histogram name
			...
			//			const int nParams=4; //number of parameters that are to be found
						...
						gMin->SetFCN(fcn);//set the address of the minimization function// fcn      CompareHists Ӻ   
						Double_t arglist[10];
						...
						textchi->SetTextAlign(12);//align = 10*HorizontalAlign + VerticalAlign, 12 meansˮƽ    롢  ֱ   ж   
						...
						graph_residual->GetYaxis()->SetRangeUser(-300, 300); 
