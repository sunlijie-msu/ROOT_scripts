...
const double Binsperkev=0.25; //Number of bins per keV, the only place to change binwidth, all other variables is related to Binsperkev // 10 for NSCL, 2 for RIBLL, useless for DSL
const int binwidth=4; //binwidths in units of keV
const int factor_rebin=4; //simu and data Rebin factor
float bkgDown=0.99;
float bkgUp=1.01;
double ULbkg=1.1;
double LLbkg=0.9;
...
// 	const int up=320; //E0_gamma+up has to be even number, if E0 is odd, up should be odd 2554
// 	const int down=140; //E0_gamma-down has to be even number, if E0 is odd, down should be odd 2094
// 	const int Ea=47;
...
// 	LLbkg=1.8/2.1;
	...
	const int up=448; //E0_gamma+up has to be even number, if E0 is odd, up should be odd 4156+448=4604/4=1151
	const int down=-112; //E0_gamma-down has to be even number, if E0 is odd, down should be odd 4156-(-112)=4268/4=1067
	const int Ea=39;
	double T0_lifetime = 0;
	int Lifetimestep = 1;
...
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)
	...
	for (flag_Eg = centroid - Eg_uncertainty*1; flag_Eg <= centroid + Eg_uncertainty*1; flag_Eg += Eg_uncertainty*1) // for (flag_Eg = centroid - Eg_uncertainty*1; flag_Eg <= centroid + Eg_uncertainty*1; flag_Eg += Eg_uncertainty*1) for all
...
//	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
		for (flag_bkg = 0.9; flag_bkg <= 1.1; flag_bkg += 0.1) // for (flag_bkg = 0.9; flag_bkg <= 1.1; flag_bkg += 0.1) for all
		{
			for (flag_SP = 0.9; flag_SP <= 1.1; flag_SP += 0.1) // for (flag_SP = 0.8; flag_SP <= 1.2; flag_SP += 0.2) for 1248 and 2234, 0.9,1.1,0.1 for others
			{
				for (flag_AC = -1.0; flag_AC <= 1.0; flag_AC += 1.0) // for (flag_AC = -0.0; flag_AC <= 0.0; flag_AC += 1.0) for 1248 and 2234, -1.0,1.0,1.0 for others
				{		
...
//  			FILE *outfile2=fopen ("D:/X/out/Compare2.dat","a"); //ofstream doesn't work. output test
			sprintf(b_name,"%s%d%s%d%s%.0f%s","D:/X/out/DSL/Chi2/Chi2_Gamma",E0_gamma,"_Ea",Ea,"_Egv",centroid,"slice_vTau_G4.dat");
...
// 	for(int iscantau=0;iscantau<=30;iscantau+=2)// all step 2186, 3435
	for(int iscantau=0;iscantau<=100;iscantau+=10) // original step 3076, 4156
//	for(int iscantau=22;iscantau<=30;iscantau+=2) // supplemental step 4156
//	for(int iscantau=200;iscantau<=1600;iscantau+=100) // all step 1248
//	for(int iscantau=50;iscantau<=550;iscantau+=50) // all step 2234
//	for(int iscantau=0;iscantau<=30;iscantau+=4)// original step 2838, 4270, 3541
//	for(int iscantau=24;iscantau<=60;iscantau+=10)// supplemental step 2838, 4270, 3541
// 					int iscantau = 400;
...
//  			hint_tau2->Draw("e3 same");
			...
			hint_tau3->Draw("e3 same"); // background uncertainty band
...
//			bkgshort->Draw("sameh");
			...
			//sprintf(paraprint,"#chi^{2} = %.2f / %d = %.5f   E_{#gamma} = %.2f keV   #tau = %.1f fs   N = %.1f +/- %.1f", Chi2,(Nbinspeak-4),Chi2/(Nbinspeak-4),flag_Eg+iscanE*binwidth,T0_lifetime+iscantau*Lifetimestep,pars[0]*21400,errs[0]*21400); //with two bumps useless now
			sprintf(paraprint,"#chi^{2} = %.2f / %d = %.5f   E_{#gamma} = %.2f keV   #tau = %.1f fs   N = %.1f +/- %.1f", Chi2,(Nbinspeak-2),Chi2/(Nbinspeak-2),flag_Eg+iscanE*binwidth,T0_lifetime+iscantau*Lifetimestep,pars[0]*21400,errs[0]*21400); //without two bumps
...
//			fprintf(outfile, "%d	%.3f", T0_lifetime+iscantau*Lifetimestep, Chi2);//Chi2 is global varible, use Chi2 to extract the minimized Chi2 obtained in fcn(), and output Chi2 in main().//DSLmodify use this sentence to output 1D2D chisquare matrix
			...
			//save figure
			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"test/S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
