...
//Enable one of the energies and others should be commented out.
	...
	const int up=162; //122 for final E0_gamma+up has to be even number, if E0 is odd, up should be odd
	const int down=58; //18 for final E0_gamma-down has to be even number, if E0 is odd, down should be odd
	const int Ea=49;
...
void Comparison_DSL_Jason_syst_test()
{
...
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)
	...
	for (flag_Eg = centroid - Eg_uncertainty*10; flag_Eg <= centroid + Eg_uncertainty*10; flag_Eg += Eg_uncertainty*10)
...
//	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
		...
		{
			for (flag_SP = 0.8; flag_SP <= 1.2; flag_SP += 0.2)
			{
...
//	for(int iscantau=50;iscantau<=550;iscantau+=50) // original step 2234 (no supplemental step)
	for(int iscantau=200;iscantau<=1400;iscantau+=200) // original step 1248
...
//			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f","D:/X/out/DSL/Eg",E0_gamma,"/S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC);
			...
			fin_data = new TFile("D:/X/out/DSL/nice_test1248_systematic_0.4k.root"); //Rootfile with Experimental Data Histogram modify
			...
			if (E0_gamma==2234&&G4bkg==0) // get rid of the low-energy Geant bkg so that pure peak component can be used to do data-to-simu comparison
			{
				...
				for (int x=2094;x<2400;x++)
				{
					...
					proton_branch[0]->SetBinContent(xbin,count-(bG4+aG4*x));
...
// 			Double_t step[4] = {0.0001,0.01,0.0001,0.0001};                //fitting step
			Double_t vstart[2] = {1.0,1.0};//initial guess par_Fit, par_bkg
			Double_t step[2] = {0.001,0.001};                //fitting step

			gMin->mnparm(0, "a1", vstart[0], step[0], 0.001,100.0,ierflg);// par for simu
...
//			bkgshort->Draw("sameh");
			...
			//sprintf(paraprint,"#chi^{2} = %.2f / %d = %.5f   E_{#gamma} = %.2f keV   #tau = %.1f fs   N = %.1f +/- %.1f", Chi2,(Nbinspeak-4),Chi2/(Nbinspeak-4),flag_Eg+iscanE*binwidth,T0_lifetime+iscantau*Lifetimestep,pars[0]*21400,errs[0]*21400); //with two bumps
			...
			//save figure
			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"test_G4/G4_S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
...
// 			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"_G4/G4_S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
...
// 			fclose(outfilehist);
// 
// 			sprintf(tname,"%s%.0f%s%04d","D:/X/out/DSL_MADAI/Eg",E0_gamma,"_G4/run",ifolder);
...
// 			fclose(outfilehistMADAI);
			...
			//sprintf(tname,"%s%.0f%s%04d","D:/X/out/DSL_MADAI_model_uncertainty/Eg",E0_gamma,"_G4/run",ifolder);
...
// 			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"_G4/S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
...
// 			fclose(outfileparaMADAI);
			...
			fprintf(outfileparaMADAIu, "Tau	%.1f\nEg	%.2f\nSP	%.2f", T0_lifetime+iscantau*Lifetimestep, flag_Eg, flag_SP);
