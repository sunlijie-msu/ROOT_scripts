...
// 			FILE *outfile2=fopen (b_name,"w"); //ofstream doesn't work
	...
	for(int iscantau=200;iscantau<=1600;iscantau+=100) // original step 1248
//	for(int iscantau=50;iscantau<=550;iscantau+=50) // original step 2234
// 	for(int iscantau=0;iscantau<=30;iscantau+=2)// all step 2186
//	for(int iscantau=30;iscantau<=100;iscantau+=10) // original step 3076, 4970, 5156, 4156
...
// 			fl->SetParLimits(1,-0.006,0.000); // DSLmodify
...
// 			fl->SetParameter(4, 2);
			...
			(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau1, 0.90);
			...
			(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau2, 0.90);
			...
			(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau3, 0.90);
...
// 			gMin->mnparm(3, "a4", vstart[3], step[3], 0.0,0.000000001,ierflg); // without two bumps
			...
			//hSega->GetYaxis()->SetRangeUser(0,hSega->GetMaximum()+17);
			
			int drawwhat=3; // modify want to drawprior =1; want to drawposterior =0; want to draw one simulated lineshape =3
			if (drawwhat==1)
			{
				TH1F *hT; // To plot prior lineshapes, i.e., simulated training sets
				for (double EEE = centroid - Eg_uncertainty*1; EEE <= centroid + Eg_uncertainty*1; EEE += Eg_uncertainty*1)
				{
					for (double bbb = 0.9; bbb <= 1.1; bbb += 0.1)
					{
						for (double sppp = 0.8; sppp <= 1.2; sppp += 0.2)
						{
							...
							{
								for (double ttt = 50.0; ttt <= 550.0; ttt += 50.0)
								{
...
// 			hlineshape_high1sigma->SetLineColor(0);
...
// 			hlineshape_low1sigma->SetLineColor(0);
...
// 				hlineshape_low1sigma->Draw("samec");
			...
			if (drawwhat==3) hFit->Draw("samehist");//hFit=p1_scale+p2_scale+p3_scale+bkgshort_scale
//			bkgshort->Draw("sameh");

			char paraprint[100];
			TPaveText *textchi = new TPaveText(0.09,0.92,0.98,0.98,"brNDC");//left, down, right, up
