...
// could add two bumps around 1248 with free parameters (optional). Three peaks + linear fline two-part bkg. Bumps have been switched off. Recommended for final results!
// could subtract 2234's low-energy Compton bkg. // It could output some final figures.
TFile *fin_simu,*fin_data,*fin_posterior_lineshape,*fin_prior_lineshape;
TH1F *hFit,*hSega,*hSega_data,*bkgshort,*background,*hSega_data_one_alpha;//
...
float bkgDown=0.99;
float bkgUp=1.00; // if you set bkgDown and bkgUp the same value, the bkg will be set to zero in minimization
double ULbkg=1.1;
double LLbkg=0.9;
...
//Enable one of the energies and others should be commented out.
	...
	const int up=162; //122 for final E0_gamma+up has to be even number, if E0 is odd, up should be odd 1410
	const int down=58; //18 for final E0_gamma-down has to be even number, if E0 is odd, down should be odd 1190
	const int Ea=49;
...
// 	const int up=290; //E0_gamma+up has to be even number, if E0 is odd, up should be odd 2554
// 	const int down=110; //E0_gamma-down has to be even number, if E0 is odd, down should be odd 2094
// 	const int Ea=47;
...
// 	const int up=300; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
...
// 	const int up=480; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-224; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=42;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=480; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-248; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=42;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=222; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-80; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=45;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=347; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
...
// 	const int up=481; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-257; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=39;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=400; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-190; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=39;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
void Comparison_DSL_best_PLBfigure()
{
// 	if (nBranches == 3) // only for 1248 test, useless for now.
// 	{
// 		TH1F *h1=new TH1F("h1","h1",4000, 0,8000);
// 		for (int i=0;i<100000;i++)
// 		{
// 			float myvalue = gRandom->Gaus(1254,3); // contaminant bump 1
// 			h1->Fill(myvalue);//Add one x-value at a time
// 		}
// 		TH1F *h2=new TH1F("h2","h2",4000, 0,8000);
// 		for (int i=0;i<100000;i++)
// 		{
// 			float myvalue = gRandom->Gaus(1283,2); // contaminant bump 2
// 			h2->Fill(myvalue);//Add one x-value at a time
// 		}
//	}
...
//		supplement 70,80,90,100 fs: start from 2025	
//	  	int ifolder = 2025; // 2025 - (2024+4*81=2348) for 70,80,90,100 fs supplemental step 4156, 5293
...
//	int ifolder = 0; // original step 3541 0-890
// 	int ifolder = 891; // supplemental step 3541 891-1052
//  for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)

// 	int ifolder = 0; // all step 4156, 5141, 5293, 2838, 4270, 3541, 0-100, 0-890
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)

	int ifolder = 0; // first step 5156 16*81=1296
	//centroid = 3076.20;
	//centroid = 4970.23;
	//centroid = 5155.54;
	for (flag_Eg = centroid - Eg_uncertainty*0; flag_Eg <= centroid + Eg_uncertainty*0; flag_Eg += Eg_uncertainty*1)
// 	int ifolder = 864; // second step 5156 16*81=1296
// 	for (flag_Eg = centroid + Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty) // second step 5156
...
//	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
...
//  			FILE *outfile2=fopen ("D:/X/out/Compare2.dat","a"); //ofstream doesn't work. output test
 			sprintf(b_name,"%s%.0f%s%d%s%.0f%s","D:/X/out/DSL/Chi2/Chi2_Gamma",E0_gamma,"_Ea",Ea,"_Egv",centroid,"slice_vTau.dat");
...
//	for(int iscantau=1000;iscantau<=1000;iscantau+=100) // original step 1248
//	for(int iscantau=250;iscantau<=250;iscantau+=50) // original step 2234
 	for(int iscantau=400;iscantau<=400;iscantau+=2)// all step 2186
//	for(int iscantau=0;iscantau<=0;iscantau+=1) // original step 3076, 4970, 5156, 4156
...
// 					int iscantau = 400;
// 	int plus_second_tau = 5000;
	{	
		if(iscantau==1100||iscantau==1300||iscantau==1500) continue;
		...
		{//shift simulated spectrum with different gamma energies 1369 i=34; 2754 i=29; 2870 i=30; 4238 i=27
			...
			char simurootname[400], outrootname[400], outtxtname[400], outfigname[400], tname[400],hname[400], ename[400], pname[400], lname[400];
			...
			//sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.2f","D:/X/out/DSL/Eg",E0_gamma,"/S31_Gamma",E0_gamma,"_Eg",4155.84,"_Tau",1.0,"_SP",1.00,"_AC",0.40);
			//read in lots of simulation root files from Oliver's simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
//			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f","D:/X/out/DSL/Eg",E0_gamma,"/G4_S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC);
			//read in lots of simulation root files from VMware G4 simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
			sprintf(simurootname,"%s%s",tname,".root");
			...
			sprintf(hname,"Eg");
			proton_branch[0]=(TH1F*)fin_simu->Get(hname);

			int G4bkg=0; // 1 means keep G4 bkg in the spectrum, 0 means subtract G4 bkg from the spectrum (preferable).
			if (E0_gamma==2234&&G4bkg==0) // get rid of the low-energy Geant bkg so that pure peak component can be used to do data-to-simu comparison
			{
				TF1 *G4pol1=new TF1("G4pol1","[0]*x+[1]",2100,2210);//G4 bkg
				proton_branch[0]->Fit("G4pol1","MLE","",2100,2210);
				proton_branch[0]->Fit("G4pol1","MLE","",2100,2210);
				proton_branch[0]->Fit("G4pol1","MLE","",2100,2210);
				double aG4 = G4pol1->GetParameter(0);
				double bG4 = G4pol1->GetParameter(1);
				for (int x=2094;x<2400;x++)
				{
					int xbin=proton_branch[0]->FindBin(x);
					int count = proton_branch[0]->GetBinContent(xbin);
					if (bG4+aG4*x<0) break;
					proton_branch[0]->SetBinContent(xbin,count-(bG4+aG4*x)); //only works for 1-keV bin size
					//cout<<x<<"	"<<count<<"	"<<(bG4+aG4*x)<<endl;
				}
			}
			if (E0_gamma==1248&&G4bkg==0) // get rid of the low-energy Geant bkg so that pure peak component can be used to do data-to-simu comparison
			{
				TF1 *G4pol1=new TF1("G4pol1","[0]*x+[1]",1154,1230);//G4 bkg
				proton_branch[0]->Fit("G4pol1","MLE","",1154,1230);
				proton_branch[0]->Fit("G4pol1","MLE","",1154,1230);
				proton_branch[0]->Fit("G4pol1","MLE","",1154,1230);
				double aG4 = G4pol1->GetParameter(0);
				double bG4 = G4pol1->GetParameter(1);
...
// 			if (nBranches == 3)
// 			{
// 				proton_branch[1]= (TH1F*)h1->Clone("bump1"); // contaminant bump 1
// 				proton_branch[2]= (TH1F*)h2->Clone("bump2"); // contaminant bump 2
// 				//proton_branch[1]->Rebin(factor_rebin);//unnecessary. contaminant spectrum is already 2-keV bin. Rebin simulated histograms from 1-keV bin to 2-keV bin because data are 2-keV bin
// 			}
...
//			for(int ii=0;ii<=12;ii+=2) // ii = 0, 2, 4, 6, 8, 10, 12; Ea = 49, 47, 45, 43, 41, 39, 37 DSLmodify six alpha gates window
			for(int ii=0;ii<=0;ii+=2) // ii = 0; Ea = 49
			{
			...
			hSega_data->Rebin(factor_rebin/2);
...
//*****************************************************************************************************
			...
			for(int i=1;i<=Nbins;i++)
			{
				...
				//if(i>=36&&i<=72)hSega->SetBinContent(i,3+0.025*proton_branch[0]->GetBinContent(((minrange-0)+Ebin_gamma-E0_gamma)/binwidth-iscanE+i));//manually add more counts to data, manipulate for fun
...
// 			TF1 *f2=new TF1("f2","[0]+x*[1]",fitrangehigh1,fitrangehigh2);
			...
			reject = kTRUE;
// 			fl->SetParLimits(0,1,2.5);
 			fl->SetParLimits(1,-0.041,-0.008); // DSLmodify
...
// 			fl->SetParameter(4, 2);
			...
			TH1D *hint_tau1 = new TH1D("hint_tau1", "Fitted func with conf.band", (fitrangelow2-fitrangelow1)*10, fitrangelow1,fitrangelow2+1);
			(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau1, 0.994);
			hint_tau1->SetStats(kFALSE);
			hint_tau1->SetFillColor(kAzure-4);

			TH1D *hint_tau2 = new TH1D("hint_tau2", "Fitted func with conf.band", (fitrangehigh2-fitrangehigh1)*10, fitrangehigh1-1,fitrangehigh2);
			(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau2, 0.963);
			hint_tau2->SetStats(kFALSE);
			hint_tau2->SetFillColor(kAzure-4);

			TH1D *hint_tau3 = new TH1D("hint_tau3", "Fitted func with conf.band", (fitrangehigh2-fitrangelow1)*10, fitrangelow1,fitrangehigh2);
			(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau3, 0.950);
...
//*****************************************************************************************************
			...
			for(int ii=0; ii<nBranches; ii++)
			{
				...
				if(ii>=2) proton_copy[ii]->SetLineColor((Color_t)(ii+49));
...
// 			Double_t step[4] = {0.0001,0.01,0.0001,0.0001};                //fitting step
			Double_t vstart[2] = {0.004,1.0};//initial guess par_Fit, par_bkg
			Double_t step[2] = {0.0001,0.01};                //fitting step

			gMin->mnparm(0, "a1", vstart[0], step[0], 0.000023,10,ierflg);// par for simu
...
//  		gMin->mnparm(2, "a3", vstart[2], step[2], 0.00005,10.0,ierflg);  // with two bumps
//  		gMin->mnparm(3, "a4", vstart[3], step[3], 0.00005,10.0,ierflg);  // with two bumps
// 			gMin->mnparm(2, "a3", vstart[2], step[2], 0.0,0.000000001,ierflg); // without two bumps
// 			gMin->mnparm(3, "a4", vstart[3], step[3], 0.0,0.000000001,ierflg); // without two bumps
			...
			for(int ii=0;ii<nBranches;ii++)
			{
				if (ii==0) proton_copy[ii]->Scale(pars[0]);//Multiply this histogram by a constant pars[0].
				if (ii==1) proton_copy[ii]->Scale(pars[2]);//Multiply this histogram by a constant pars[2].
				if (ii==2) proton_copy[ii]->Scale(pars[3]);//Multiply this histogram by a constant pars[3].
				proton_copy[ii]->Add(bkgshort,pars[1]); //Multiply the background histogram by a constant pars[1].
			}
			// 	p1->Scale(pars[0]);
			...
			c1->cd();
			TPad *pad1 = new TPad("pad1", "The pad 70% of the height",0.0,0.0,1.0,1.0);// Double_t xlow, Double_t ylow, Double_t xup, Double_t yup,
			...
			hSega->GetXaxis()->SetLabelSize(0.05);
			hSega->GetXaxis()->CenterTitle();
			hSega->GetYaxis()->CenterTitle();
			hSega->GetXaxis()->SetTitleOffset(1.2);
			hSega->GetYaxis()->SetTitleOffset(0.8);
			hSega->GetXaxis()->SetTitleSize(0.05);
			hSega->GetYaxis()->SetTitleSize(0.05);
			hSega->GetYaxis()->SetLabelSize(0.05);
			hSega->GetXaxis()->SetNdivisions(505);//n = n1 + 100*n2 + 10000*n3
			hSega->GetYaxis()->SetNdivisions(505);//n = n1 + 100*n2 + 10000*n3
			hSega->GetXaxis()->SetRangeUser(minrangezoom,maxrangezoom);
			
			int drawprior=0; // modify want to drawprior =1; want to drawposterior =0.
			if (drawprior==1)
			{
				TH1F *hT; // To plot prior lineshapes, i.e., simulated training sets
				for (double EEE = centroid - Eg_uncertainty*0; EEE <= centroid + Eg_uncertainty*0; EEE += Eg_uncertainty*10)
				{
					for (double bbb = 0.9; bbb <= 1.1; bbb += 0.1)
					{
						for (double sppp = 0.8; sppp <= 1.2; sppp += 0.2)
						{
							for (double aaa = 0.0; aaa <= 0.0; aaa += 1.0)
							{
								for (double ttt = 200.0; ttt <= 1600.0; ttt += 100.0)
								{
									if(ttt==1100||ttt==1300||ttt==1500) continue;
									sprintf(lname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f%s","D:/X/out/DSL_Comparison/Eg",E0_gamma,"/S31_Gamma",E0_gamma,"_Eg",EEE,"_Tau",ttt,"_SP",sppp,"_AC",aaa,"_bkg",bbb,".root");
									fin_prior_lineshape = new TFile(lname); //Rootfile with prior lineshapes, i.e., simulated training sets
									hT=(TH1F*)fin_prior_lineshape->Get("hFit");
									hT->SetLineColorAlpha(kRed-7,1.0);
									hT->SetLineWidth(9);
									if (bbb==1.0) hT->SetLineWidth(11);
									//if (bbb==0.9) hT->SetFillColor(10);
									for (int ii=1; ii<=hT->GetNbinsX(); ii++)
									{
										hT->SetBinError(ii,0);
									}
									hT->Draw("same c");
...
// 			hint_tau3->Draw("e3 same"); // background uncertainty band DSLmodify
			bkgshort->SetLineWidth(2);

			sprintf(lname,"%s%.0f%s","D:/X/out/DSL_Final_Tau/Posterior_lineshape",E0_gamma,".root");
			fin_posterior_lineshape = new TFile(lname); //Rootfile with Posterior lineshapes
...
// 			hlineshape_high->SetLineColor(0);
			...
			TH1F *hlineshape_high1sigma=new TH1F(*hlineshape_high1sigma);
			hlineshape_high1sigma->SetFillColorAlpha(kAzure-4,1.0);
			hlineshape_high1sigma->SetLineColor(kAzure-4);
...
// 			hlineshape_low->SetLineColor(0);
			...
			TH1F *hlineshape_low1sigma=new TH1F(*hlineshape_low1sigma);
			hlineshape_low1sigma->SetFillColor(10);
			hlineshape_low1sigma->SetLineColor(kAzure-4);

			if (drawprior==0)
			{
				hint_tau1->Draw("e3 same");
				hint_tau2->Draw("e3 same");
				hlineshape_high1sigma->Draw("samec");
				hlineshape_low1sigma->Draw("samec");
// 				hlineshape_high->Draw("samec"); //2sigma
// 				hlineshape_low->Draw("samec"); //2sigma
			}
			pad1->RedrawAxis(); //In case multiple color filled histograms are drawn on the same pad, the fill area may hide the axis tick marks. One can force a redraw of the axis over all the histograms by calling
			hSega->Draw("e same"); // bands don't seem to be transparent. I have to redraw the data points above bands.
...
//			bkgshort->Draw("sameh");
			...
			char paraprint[100];
			TPaveText *textchi = new TPaveText(0.11,0.92,0.98,0.98,"brNDC");//left, down, right, up
			...
			//sprintf(paraprint,"#chi^{2} = %.2f / %d = %.5f   E_{#gamma} = %.2f keV   #tau = %.1f fs   N = %.1f +/- %.1f", Chi2,(Nbinspeak-4),Chi2/(Nbinspeak-4),flag_Eg+iscanE*binwidth,T0_lifetime+iscantau*Lifetimestep,pars[0]*21400,errs[0]*21400); //with two bumps useless
			//sprintf(paraprint,"#chi^{2} = %.2f / %d = %.5f   E_{#gamma} = %.2f keV   #tau = %.1f fs   N = %.1f +/- %.1f", Chi2,(Nbinspeak-2),Chi2/(Nbinspeak-2),flag_Eg+iscanE*binwidth,T0_lifetime+iscantau*Lifetimestep,pars[0]*20000,errs[0]*20000); //without two bumps
			//sprintf(paraprint,"#chi_{#it{#nu}}^{2} = %.2f    #it{p}-value = 0.006    #it{#tau} = %.0f fs", Chi2/(Nbinspeak-2),T0_lifetime+iscantau*Lifetimestep); //p-value from web calculator http://courses.atlas.illinois.edu/spring2016/STAT/STAT200/pchisq.html
			textchi->AddText(paraprint);
			//textchi->Draw();//write chi2 on the plot
			...
			for(int i=1;i<=Nbins;i++)
			{
				...
				Resids[i-1] = hSega->GetBinContent(i) - hFit->GetBinContent(i);//data-simu //residual
				...
				hResid->AddBinContent(i,Resids[i-1]);//data-simu
				//ResidsUncert[i-1]= pow(hSega->GetBinContent(i),0.5);
				ResidsUncert[i-1]= (hSega->GetBinErrorUp(i)+hSega->GetBinErrorLow(i))/2.0;

				hResid->SetBinError(i,ResidsUncert[i-1]);//data-simu
				//hResid->Sumw2(kFALSE);
			...
			hResid->GetXaxis()->SetTitleSize(0.12);
			hResid->GetXaxis()->SetLabelOffset(0.015);
			hResid->GetXaxis()->SetLabelSize(0.12);
			hResid->GetYaxis()->SetLabelSize(0.12);
			hResid->GetYaxis()->SetTitleSize(0.12);
			hResid->GetXaxis()->SetNdivisions(505);//n = n1 + 100*n2 + 10000*n3
			...
			fprintf(outfile, "%d	%.3f", T0_lifetime+iscantau*Lifetimestep, Chi2);//Chi2 is global varible, use Chi2 to extract the minimized Chi2 obtained in fcn(), and output Chi2 in main().//DSLmodify use this sentence to output 1D2D chisquare matrix
			//fprintf(outfile, "Ea=	%d	tau=	%.1f	E0=	%.3f	Emax=	%.3f	-2lnL =	%.3f	-2lnL/NDF =	%.5f	%d\n", Ea,T0_lifetime+iscantau*Lifetimestep,centroid,centroid+iscanE*binwidth,Chi2,Chi2/(Nbinsz-2),Nbinsz-2);//Chi2 is global varible, use Chi2 to extract the minimized Chi2 obtained in fcn(), and output Chi2 in main().

			//save figure
			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.2f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"/S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
...
// 			//sprintf(tname,"%s%.0f%s%04d","D:/X/out/DSL_MADAI_model_uncertainty/Eg",E0_gamma,"_test_fewer_parameters/run",ifolder);
...
// 			//cout << hFit->GetBinCenter(ibin) << "	" << hFit->GetBinContent(ibin) << endl;
// 			if (E0_gamma==1248||E0_gamma==2234) fprintf(outfileparaMADAIu, "Tau	%.1f\nEg	%.2f\nbkg	%.1f\nSP	%.2f", T0_lifetime+iscantau*Lifetimestep, flag_Eg, flag_bkg, flag_SP);
//			else fprintf(outfileparaMADAIu, "Tau	%.1f\nEg	%.2f\nbkg	%.1f\nSP	%.2f\nAC	%.1f", T0_lifetime+iscantau*Lifetimestep, flag_Eg, flag_bkg, flag_SP, flag_AC);
// 			fclose(outfileparaMADAIu);
		...
		fprintf(outfile, "\n");//DSLmodify use this sentence to output 1D2D chisquare matrix
...
{
	if (hFit) delete hFit;
	hFit = (TH1F*)proton_copy[0]->Clone("hFit");//Making a copy of histogram p1, rename it "hFit"
	hFit->Scale(par[0]);  //Scale this histogram by a constant (relative intensity), determined by the minuit above
	...
	for(int ii=1;ii<nBranches;ii++)
	{
		if (ii==1) hFit->Add(proton_copy[ii],par[2]);//p2 relative intensity is determined by the minuit above
		if (ii==2) hFit->Add(proton_copy[ii],par[3]);//p3 relative intensity is determined by the minuit above
	}
