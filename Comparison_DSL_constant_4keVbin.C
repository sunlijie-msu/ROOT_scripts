...
// 	const int up=122; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=18; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=49;
...
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)
	...
	for (flag_Eg = centroid - Eg_uncertainty*0; flag_Eg <= centroid + Eg_uncertainty*0; flag_Eg += Eg_uncertainty*10) // first step 5156
...
//	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
		...
		{
			for (flag_SP = 1.00; flag_SP <= 1.00; flag_SP += 0.2)
			{
...
//	for(int iscantau=0;iscantau<=500;iscantau+=20) // original step 2234 (no supplemental step)
	for(int iscantau=50;iscantau<=400;iscantau+=50) // original step 1248
...
//				double c = (fl->GetParameter(0)+(minrangeb+(double)i/Binsperkev)*fl->GetParameter(1));//HE BG
  				double a = (fl->GetParameter(0));//Lower BG //Chris likes 3
//  				double c = (f2->GetParameter(0));//Upper BG //Chris likes 4
