...
using namespace std;
//main_readhists; fcn(); comparehists(); main_draw_save;//search m o d i f y to change peak. I don't think this code is very useful now.
...
// could subtract 2234's low-energy Compton bkg.
...
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + Eg_uncertainty; flag_Eg += Eg_uncertainty)
	...
	for (flag_Eg = 1248.50; flag_Eg <= 1248.50; flag_Eg += Eg_uncertainty*10)
...
//	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
		...
		{
			for (flag_SP = 0.98; flag_SP <= 0.98; flag_SP += 0.2)
			{
...
//	for(int iscantau=0;iscantau<=500;iscantau+=20) // original step 2234 (no supplemental step)
	for(int iscantau=715;iscantau<=715;iscantau+=50) // original step 1248
