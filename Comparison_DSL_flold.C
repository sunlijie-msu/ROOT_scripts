...
//outdated, but used for the final results for 3076, 4971, 5157, etc except for 1248, 2234
//now it outputs some final figures
TFile *fin_simu,*fin_data;
TH1F *hFit,*hSega,*hSega_data,*bkgshort,*background;// no hSega_data_one_alpha in the freepeak.C
...
// 	const int up=304; //354 E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-120; //-70 E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=46;
...
// 	const int up=462; // 532 E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-238; //-154 E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=42;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=490; // 570 E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-260; //-198 E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=42;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=222; // 242 E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-80; //-40 E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=45;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=345; //415 E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-161; //-71 E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=40;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
// 	const int up=390; //450 E0_gamma+up has to be even number, if E0 is odd, up should be odd
// 	const int down=-210; //-110 E0_gamma-down has to be even number, if E0 is odd, down should be odd
// 	const int Ea=39;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
//  	int ifolder = 1701; // 1701-1781 for 30fs, 1782-1862 for 40fs, 1863-1943 for 50 fs, 1944-2024 for 60 fs supplemental step 4156, 5293
...
//	int ifolder = 891; // supplemental step 3541 891-1052
 for (flag_Eg = centroid - Eg_uncertainty*0; flag_Eg <= centroid + Eg_uncertainty*0; flag_Eg += Eg_uncertainty)
...
// 	for(int iscantau=0;iscantau<=100;iscantau+=10)// all step 5293, 4156, 5141, 2838, 4270
//	for(int iscantau=0;iscantau<=50;iscantau+=5)// first step 3541
	for(int iscantau=0;iscantau<=40;iscantau+=4)// second step 3541
//	for(int iscantau=0;iscantau<=20;iscantau+=1) // original step 3076, 4156
//	for(int iscantau=0;iscantau<=30;iscantau+=2) // all step 5156
//	for(int iscantau=30;iscantau<=60;iscantau+=10) // supplemental step 4156
//	for(int iscantau=70;iscantau<=100;iscantau+=10) // supplemental step 4156
//	for(int iscantau=0;iscantau<=500;iscantau+=20) // original step 2234 (no supplemental step)
//	for(int iscantau=600;iscantau<=6000;iscantau+=200) // original step 1248
...
//	for(int iscantau=70;iscantau<=100;iscantau+=10)// supplemental step 2838, 4270, 3541
	{	
...
// 			fclose(outfileparaMADAIu);
		...
		//fprintf(outfile2, "\n");//DSLmodify use this sentence to output 1D2D Bayesian matrix
	...
	//iflag flag word to switch between several actions of FCN	//Usually only f and par are essential
{
	...
	for(int ii=1;ii<nBranches;ii++)
	{
		hFit->Add(proton_copy[ii],1); //p2 relative intensity is determined by the minuit above
	}
