...
// manually add more counts to data, manipulate for fun
...
float bkgDown=0.89;
float bkgUp=0.90; // if you set bkgDown and bkgUp the same value, the bkg will be set to zero in minimization
double ULbkg=1.1;
double LLbkg=0.9;
...
void Comparison_DSL_manipulate()
{
...
//  			FILE *outfile2=fopen ("D:/X/out/Compare2.dat","a"); //ofstream doesn't work. output test
 			sprintf(b_name,"%s%d%s%d%s%.0f%s","D:/X/out/DSL/Chi2/Chi2_Gamma",E0_gamma,"_Ea",Ea,"_Egv",centroid,"slice_vTau.dat");
...
//			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f","D:/X/out/DSL/Eg",E0_gamma,"/G4_S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC);
			...
			fin_data = new TFile("D:/X/out/DSL/manipulate.root"); //Rootfile with Experimental Data Histogram modify
...
// 			fl->SetParLimits(1,-0.005,0); // DSLmodify
