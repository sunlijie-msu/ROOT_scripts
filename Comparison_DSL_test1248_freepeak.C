...
// add two bumps around 1248 with free parameters, three peaks + bkg good!
...
const int nBranches=3;                  //13,19;2,3 Number of proton Branches feeding daughter modify
...
void Comparison_DSL_test1248_freepeak()
{
...
// 	for (flag_Eg = centroid - Eg_uncertainty; flag_Eg <= centroid + 0; flag_Eg += Eg_uncertainty) // first step 5156
...
//	int ifolder = 3024; flag_Eg = centroid - Eg_uncertainty*5; //1248 supplement 1247.40
	int ifolder =0; flag_Eg = centroid;//-Eg_uncertainty;
	{
		...
		{
			for (flag_SP = 1.02; flag_SP <= 1.02; flag_SP += 0.1)
			{
...
//	for(int iscantau=0;iscantau<=500;iscantau+=20) // original step 2234 (no supplemental step)
	for(int iscantau=800;iscantau<=800;iscantau+=200) // original step 1248
...
//  			hint_tau2->Draw("e3 same");
			...
			//save figure
			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"test/G4_S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
