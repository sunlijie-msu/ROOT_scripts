...
// manually added two bumps, useless, bad
...
//	for(int iscantau=0;iscantau<=500;iscantau+=20) // original step 2234 (no supplemental step)
	for(int iscantau=1500;iscantau<=1500;iscantau+=500) // original step 1248
...
// 			sprintf(simurootname,"%s%s",tname,".root"); //read in lots of simulation root files from Oliver's simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
// 			fin_simu = new TFile(simurootname); //Rootfile with Simulation histograms
			...
			proton_branch[1]= (TH1F*)h1->Clone("h1");
			proton_branch[2]= (TH1F*)h2->Clone("h2");
...
//*****************************************************************************************************
			...
			for(int ii=0;ii<nBranches;ii++)
			{
				...
				if (ii==1) proton_copy[ii]->Scale(pars[0]*0.02);//Multiply this histogram by two constant pars[0] and 0.02 (manually added).
				if (ii==2) proton_copy[ii]->Scale(pars[0]*0.015);//Multiply this histogram by two constant pars[0] and 0.015 (manually added).
	...
	//iflag flag word to switch between several actions of FCN	//Usually only f and par are essential
{
	...
	for(int ii=1;ii<nBranches;ii++)
	{
		if (ii==1) hFit->Add(proton_copy[ii],0.02);//p2 p3的相对intensity都用文献值比
		if (ii==2) hFit->Add(proton_copy[ii],0.015);//p2 p3的相对intensity都用文献值比
	}
	hFit->Scale(par[0]);  //Multiply thfis histogram by a constant
