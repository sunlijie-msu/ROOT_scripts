...
void Comparison_DSL_test1248_twopeakSP()
{
...
//	for(int iscantau=30;iscantau<=60;iscantau+=10)// supplemental step 2838, 4270, 3541
	{	
		...
		{//shift simulated spectrum with different gamma energies 1369 i=34; 2754 i=29; 2870 i=30; 4238 i=27
			...
			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f","D:/X/out/DSL/Eg",E0_gamma,"/S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",second_SP,"_AC",flag_AC);
			sprintf(simurootname,"%s%s",tname,".root"); //read in lots of simulation root files from Oliver's G4 simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
...
//  			hint_tau2->Draw("e3 same");
			...
			//save figure
			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/Eg",E0_gamma,"test/S31_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_",T0_lifetime+iscantau*Lifetimestep,"_SP",second_SP,"_AC",flag_AC,"_bkg",flag_bkg);
