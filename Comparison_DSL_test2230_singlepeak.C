...
// 	LLbkg=2.7/3.2;
	...
	const double E0_gamma=2230; //Ex=2230
	const int startpeak=2218; //has to be even number
	const int endpeak=2416; //has to be even number
	const int up=316; //E0_gamma+up has to be even number, if E0 is odd, up should be odd
	const int down=144; //E0_gamma-down has to be even number, if E0 is odd, down should be odd
	const int Ea=47;
	double T0_lifetime = 0; //=80
	int Lifetimestep = 1;
	double centroid=2230.49;
	double Eg_uncertainty = 0.20;
	int Yaxisrange=20;
	ULbkg=3.3/2.85;
	LLbkg=2.5/2.85;
...
//  			FILE *outfile2=fopen ("D:/X/out/Compare2.dat","a"); //ofstream doesn't work. output test
			sprintf(b_name,"%s%d%s%d%s%.0f%s","D:/X/out/DSL/Chi2/Chi2_Gamma",E0_gamma,"_Ea",Ea,"_Egv",centroid,"slice_vTau_test.dat");
...
//	for(int iscantau=0;iscantau<=500;iscantau+=20) // original step 2234 (no supplemental step)
	for(int iscantau=244;iscantau<=244;iscantau+=50) // original step 1248
...
//			sprintf(tname,"%s%.0f%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f","D:/X/out/DSL/Eg",E0_gamma,"/S32_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC);
			sprintf(tname,"%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f","D:/X/out/DSL/test/S32_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC);
			sprintf(simurootname,"%s%s",tname,"_test.root"); //read in lots of simulation root files from Oliver's simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
			fin_simu = new TFile(simurootname); //Rootfile with Simulation histograms

// 			sprintf(tname,"%s%.0f%s%.0f%","D:/X/out/DSL/Eg",E0_gamma,"/ExG4_Tau",T0_lifetime+iscantau*Lifetimestep);
// 			sprintf(simurootname,"%s%s",tname,".root"); //read in lots of simulation root files from VMware G4 simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
// 			fin_simu = new TFile(simurootname); //Rootfile with Simulation histograms
			...
			{
				sprintf(hname,"Eg"); // Oliver's
//				sprintf(hname,"h1QSD1e"); // VMware
...
// 			hSega_data= (TH1F*)fin_data->Get(hname); //Get Gamma ray spectrum
			...
			hSega_data = new TH1F("hSega_data","hSega_data",4750,0,9500);
//			for(int ii=0;ii<=12;ii+=2) // ii = 0, 2, 4, 6, 8, 10, 12; Ea = 49, 47, 45, 43, 41, 39, 37 DSLmodify six alpha gates window
			for(int ii=0;ii<=0;ii+=2) // ii = 0; Ea = 49
			{
				...
				//sprintf(hname,"%s","hGriffinAddback_he3ex_t0");//particle gate
				sprintf(hname,"%s","hGriffinAddback_alpha_t0");//particle gate
...
//			hint_tau3->Draw("e3 same");
			...
			sprintf(paraprint,"#chi^{2} = %.2f / %d = %.5f   E_{#gamma} = %.2f keV   #tau = %.1f fs   N = %.1f +/- %.1f", Chi2,(Nbinspeak-2),Chi2/(Nbinspeak-2),flag_Eg+iscanE*binwidth,T0_lifetime+iscantau*Lifetimestep,pars[0]*3200,errs[0]*3200);//only for DSL low stat peaks
			...
			//save figure
			sprintf(tname,"%s%.0f%s%.2f%s%.1f%s%.2f%s%.1f%s%.1f","D:/X/out/DSL_Comparison/test/S32_Gamma",E0_gamma,"_Eg",flag_Eg,"_Tau",T0_lifetime+iscantau*Lifetimestep,"_SP",flag_SP,"_AC",flag_AC,"_bkg",flag_bkg);
