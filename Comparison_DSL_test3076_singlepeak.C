...
using namespace std;
//main_readhists; fcn(); comparehists(); main_draw_save;//search testmodify to change distance
...
// 	const int startpeak=4154; //has to be even number
...
// 	const int down=100; //E0_gamma-down has to be even number, if E0 is odd, down should be odd. original down= -110
// 	const int Ea=39;
// 	double T0_lifetime = 0;
// 	int Lifetimestep = 1;
...
void Comparison_DSL_test3076_singlepeak()
{
...
// 			FILE *outfile=fopen (b_name,"w"); //ofstream doesn't work
			sprintf(b_name,"%s%d%s%d%s%.0f%s%s%s","D:/X/out/DSL/LBayesian/LBayesian_Gamma",E0_gamma,"_Ea",Ea,"_Egv",centroid,"slice_vTau_coll","24",".dat"); //testmodify
...
//	for(int iscantau=00;iscantau<=300;iscantau+=100) // original step 1248
...
// 	int plus_second_tau = 5000;
	{	
		...
		{//shift simulated spectrum with different gamma energies 1369 i=34; 2754 i=29; 2870 i=30; 4238 i=27
			...
			sprintf(simurootname,"%s%s",tname,"_colldis24.root"); //read in lots of simulation root files from Oliver's G4 simulation. Input root file names don't contain flag_bkg, but output fig/dat/root file names contain flag_bkg.
			//testmodify
