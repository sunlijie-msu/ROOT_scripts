...
#include "TLegend.h"
using namespace std;
void Fill_2D_from_txt_Tau_3fs_OmegaGamma()//read bincontent from a txt file and create a histo and save it as a new root file.
{// input bin by bin, not event by event
	char outrootname[200], infilename[200], outfigname[200], tname[200];
	int Entries=300;//modify
	sprintf(tname, "%s", "D:/X/out/Bayesian_VS/Resonance_Strength/Tau_3fs_OmegaGamma_Joint");
	...
	TH1F* hTau = new TH1F("hTau", "hTau", 150, 0, 12);
	TH1F* hOmegaGamma = new TH1F("hOmegaGamma", "hOmegaGamma", 800, 0, 200);
	TH2F* hTauOmegaGamma = new TH2F("hTauOmegaGamma", "hTauOmegaGamma", 150, 0, 12, 800, 0, 200); // TH2(name, title, nbinsx, xlow, xup, nbinsy, ylow, yup);
	...
	TLine* LineOmegaGamma = new TLine(0, 35.450173386103586, 12, 35.450173386103586); // TLine(x1,y1,x2,y2)
	LineOmegaGamma->SetLineColor(kRed);
	LineOmegaGamma->SetLineWidth(2);
	LineOmegaGamma->SetLineStyle(7);
	LineOmegaGamma->Draw("");

	TLine* LineOmegaGammaLow = new TLine(0, 22.379846928242433, 12, 22.379846928242433); // TLine(x1,y1,x2,y2)
	LineOmegaGammaLow->SetLineColor(kBlue);
	LineOmegaGammaLow->SetLineWidth(2);
	LineOmegaGammaLow->SetLineStyle(7);
	LineOmegaGammaLow->Draw("");

	TLine* LineOmegaGammaHigh = new TLine(0, 61.62212264632808, 12, 61.62212264632808); // TLine(x1,y1,x2,y2)
	LineOmegaGammaHigh->SetLineColor(kBlue);
	LineOmegaGammaHigh->SetLineWidth(2);
	LineOmegaGammaHigh->SetLineStyle(7);
	LineOmegaGammaHigh->Draw("");

	TLine* LineTau = new TLine(3.0747882567207863, 0, 3.0747882567207863, 200); // TLine(x1,y1,x2,y2)
	LineTau->SetLineColor(kRed);
	LineTau->SetLineWidth(2);
	LineTau->SetLineStyle(7);
	LineTau->Draw("");

	TLine* LineTauLow = new TLine(1.811800012098507, 0, 1.811800012098507, 200); // TLine(x1,y1,x2,y2)
	LineTauLow->SetLineColor(kBlue);
	LineTauLow->SetLineWidth(2);
	LineTauLow->SetLineStyle(7);
	LineTauLow->Draw("");

	TLine* LineTauHigh = new TLine(4.701251738588116, 0, 4.701251738588116, 200); // TLine(x1,y1,x2,y2)
	LineTauHigh->SetLineColor(kBlue);
	LineTauHigh->SetLineWidth(2);
	LineTauHigh->SetLineStyle(7);
	LineTauHigh->Draw("");
