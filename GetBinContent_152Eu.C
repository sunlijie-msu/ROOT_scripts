...
#include "TLegend.h"
using namespace std;
void GetBinContent_152Eu()//get lots of peak integrals from lots of histograms
{
...
//	int Eg[12]={451,1599,2904,7801,7801,7801,7801,7801,7801,7801,7801,7801};//23Al
	...
	for(i=0;i<16;i++)//which SeGA detector
	{
		...
		histo[i] = (TH1F*)fin->Get(hname);
		outfile<<hname<<endl;
		for(ii=0;ii<=8;ii++)//which peak
		{
			for (jj=-numberofbin3;jj<=numberofbin3;jj++)
			{
				Integral[ii]+=histo[i]->GetBinContent(Eg[ii]+jj);//-3+3
				//cout<<Eg[ii]+jj<<'	';
				bkgl[ii]+=histo[i]->GetBinContent(Eg[ii]-numberofbin3*2+jj-1);//-10-4
				//cout<<Eg[ii]-numberofbin10*2+jj-1<<'	';
				bkgh[ii]+=histo[i]->GetBinContent(Eg[ii]+numberofbin3*2+jj+1);//+4+10
				//cout<<Eg[ii]+numberofbin10*2+jj+1<<endl;
			}
			outfile<<"Eg=	"<<Eg[ii]<<"	Integral=	"<<Integral[ii]<<"	bkgl=	"<<bkgl[ii]<<"	bkgh=	"<<bkgh[ii]<<endl;
		}
