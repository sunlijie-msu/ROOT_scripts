...
using namespace std;
void GetBinContentandError_4keVbin_4156()//get BinContent and BinError from DSL gamma-ray spectra data histograms for MADAI observables
{
	int ii,jj,i,j;
	char calrootname[80];
...
int Eg_end[13]={1352, 2420, 3326, 5370, 5584, 3716, 2364, 3070, 4610, 3834, 5554, 4494, 5720};
	int down[13]={ 100,   140,   -70,    -154,  -198,   -81,    -40,    -66,   -126,    -71,   -181,  -112, -151};
	...
	for (int Ea=39; Ea<=39; Ea++)
	{
		sprintf(hname,"%s%d%s","D:/X/out/DSL/nice_Ea",Ea,"_4keVbin.dat");//alpha gate
...
// 	}
	...
	for (int j=11; j<12; j++)
	{
		sprintf(hname,"%s%d%s","D:/X/out/DSL/experimental_results_MADAI_",Eg[j],"_4keVbin.dat");//Eg
		...
		for (int j=11; j<12; j++)
		{
			sprintf(hname,"%s%d%s","D:/X/out/DSL/observable_names_MADAI_",Eg[j],"_4keVbin.dat");//Eg
