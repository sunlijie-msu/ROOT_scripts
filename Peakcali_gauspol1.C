...
void Peakcali_gauspol1()
{
	...
	const int ID1=3;//adjust
	const int ID2=4;//adjust
	const int binmin=600,binmax=1000;//adjust//search for peaks in this range
	const int binwidth=2;
	int binnum=(binmax-binmin)/binwidth;
	double sigma=4,thresh=0.05,sigmab=30;//adjust
	float gaplow=25.,gaphigh=35.;//fitting range随分辨不同调整//adjust
	unsigned long i;
	int jj,ii;
	...
	unsigned long nentries=T777->GetEntries();//读事件数
	cout<<rawrootname;
	...
	sprintf(b_name,"%s","LaBr3");
	//cin>>b_name;//input the branch name you want to search its peaks
	// 	while(strcmp(b_name,"D142ALch")!=0&&strcmp(b_name,"D142BLch")!=0&&strcmp(b_name,"D40ALch")!=0&&strcmp(b_name,"D40BLch")!=0&&strcmp(b_name,"D304ALch")!=0&&strcmp(b_name,"D304BLch")!=0&&strcmp(b_name,"LaBr3")!=0)
	// 	{
	...
	for(ii=ID1;ii<ID2;ii++)
	{
		...
		puts(command);
		T777->Draw(command);//T999-Draw("DSSD1AL[0]>>hDSSD1AL[0]")
	}
	...
	for(i=ID1;i<ID2;i++)
	{
		...
		//TH1F *histob = s->Background(histo[i], sigmab, "same");//if you do not want the background subtraction, annotate this statement
		//histo[i]->Add(histob, -1);//if you do not want the background subtraction, annotate this statement
		...
		for(ii=0;ii<peaknum;ii++)
		{
			...
			total[ii]->SetParLimits(0,50,100000);//Constant,min,max
			total[ii]->SetParLimits(1,50,8000);//Mean
			total[ii]->SetParLimits(2,1,50);//Sigma
			//total[ii]->SetParLimits(3,-1000,1000);//B
			//total[ii]->SetParLimits(4,0,0);//A
			total[ii]->SetParNames("Const","Mean","Sigma","B","A");
			...
			total[ii]->GetParameters(par[ii]);//total[ii],pointer to the TF1, GetParameters的数组得是double类型Obtaining the value of parameters and saving them to par[]; 
		...
		for(ii=0;ii<peaknum;ii++)
		{
			sprintf(paraprint,"Mean%d=%.1f",ii,par[ii][1]);//par数组还保持着刚才的参数
