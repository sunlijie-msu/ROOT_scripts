...
#include "TLegend.h"
using namespace std;
void Peakcali_treedrawBS()//for calibrating the DSSD1BS, DSSD2AS, DSSD3BS
{
	time_t tim;
	struct tm *at;
	char now[80];
	int irawroot;
	char calrootname[120];
	char calrootname[120];
	double slope[300];
	double intercept[300];
	const int ID1=7;//adjust 0
	const int ID2=16;//adjust 16
	int binmin=3000,binmax=3500;//adjust//search for peaks in this range
	double sigma=50,thresh=0.3,sigmab=140;//adjust
	float gaplow=50.,gaphigh=50.;//fitting range随分辨不同调整//adjust
	unsigned long i;
	int jj,ii;
	...
	int peaknum;
	//float *peakmean;
	const int nummax=4;//search peaknumber can be many, fitnumber can be limited to 3//adjust
	double par[12][3];//随峰数不同改动par[peaknum][3]//adjust
	float energy[4]={121.16,112.7,104.81,97.45};
	...
	//{
		...
		sprintf(calrootname,"X:/T777/S27calabcd0252.root");//modify if change path
		T777Chain->Add(calrootname);
		sprintf(calrootname,"X:/T777/S27calabcd0253.root");//modify if change path
		T777Chain->Add(calrootname);
		sprintf(calrootname,"X:/T777/S27calabcd0254.root");//modify if change path
		T777Chain->Add(calrootname);
		sprintf(calrootname,"X:/T777/S27calabcd0255.root");//modify if change path
		T777Chain->Add(calrootname);
		sprintf(calrootname,"X:/T777/S27calabcd0256.root");//modify if change path
		T777Chain->Add(calrootname);
		...
		unsigned long nentries=T777Chain->GetEntries();//读事件数
		//cout<<calrootname;
		...
		sprintf(b_name,"%s","DSSD3BSE");
		//cin>>b_name;//input the branch name you want to search its peaks
		while(strcmp(b_name,"DSSD1BSE")!=0&&strcmp(b_name,"DSSD3BSE")!=0&&strcmp(b_name,"DSSD2ASE")!=0)
		{
		...
		for(ii=ID1;ii<ID2;ii++)
		{
			...
			histo[ii] = new TH1F(histo_name,histo_name,600,1,6001);//name a histogram,hDSSD1AL[0]//modify
		...
		for(ii=ID1;ii<ID2;ii++)
		{
				...
				puts(command1);
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-181.55)*(TOF-181.55)/(1.33*1.33))+((DE1max-202.4)*(DE1max-202.4)/(12*12))<=1");//29S
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-186.7)*(TOF-186.7)/(1.33*1.33))+((DE1max-187.5)*(DE1max-187.5)/(12*12))<=1");//28P
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-192.4)*(TOF-192.4)/(1.5*1.5))+((DE1max-173.9)*(DE1max-173.9)/(12.5*12.5))<=1");//27Si
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-199.2)*(TOF-199.2)/(1.3*1.3))+((DE1max-160.2)*(DE1max-160.2)/(8*8))<=1");//26Al
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-198.9)*(TOF-198.9)/(1.3*1.3))+((DE1max-139)*(DE1max-139)/(8*8))<=1");//24Mg
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-207.3)*(TOF-207.3)/(1.5*1.5))+((DE1max-127)*(DE1max-127)/(9*9))<=1");//23Na
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-217.6)*(TOF-217.6)/(1.5*1.5))+((DE1max-116)*(DE1max-116)/(9*9))<=1");//22Ne
				sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-199)*(TOF-199)/(1.2*1.2))+((DE1max-100)*(DE1max-100)/(6*6))<=1");//20Ne
				//sprintf(command2,"%s%s%d%s",b_name,"[",ii,"]>50&&((TOF-199)*(TOF-199)/(1.2*1.2))+((DE1max-70)*(DE1max-70)/(5*5))<=1");//16O
				puts(command2);
				T777Chain->Draw(command1,command2);//T999-Draw("DSSD1AL[0]>>hDSSD1AL[0]")
		}
...
// 		}//for(i=0;i<nentries;i++) △treedraw
		...
		ofstream outfile("X:/peakcali.dat",ios::out);//modify if change path
		for(i=ID1;i<ID2;i++)
		{
			...
			for(ii=0;ii<peaknum;ii++)
			{
				...
				histo[i]->Fit("g","R+");//restrict the fit to the range specified in the TF1 constructor: peakch[ii]-gaplow,peakch[ii]+gaphigh
			...
			sprintf(hfit_name,"%s%s%s","X:/",hfit_name,".png");//modify if change path
			canvaspeak[i]->SaveAs(hfit_name);//存图
			for(ii=0;ii<peaknum;ii++)
			{
				outfile<<b_name<<i<<" Mean"<<ii<<"=	"<<par[ii][1]<<endl;//输出文本查看
				peakch[ii]=par[ii][1];// For graph pol fit, mean is more accurate than peakch
			}
			...
			sprintf(hcali_name,"%s%s%s","X:/",hcali_name,".png");//modify if change path
