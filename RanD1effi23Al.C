...
#include <time.h>
using namespace std;
void RanD1effi23Al()//能量相关，从D1发射一定能量质子，被各探测器探测到的概率
{
	...
	sprintf(anarootname,"V:/RIBLL2015/data24/Al23calabcd_0636_0650p10T_NIMA.root");
...
// 	}
	sprintf(simurootname,"%s","V:/RIBLL2015/data24/Al23simulate.root");
...
// 	h3->FillRandom("gaus",5000);
	...
	for(ii=0;ii<23;ii++)
	{
		infile>>PrangeinSi[ii]>>PdrangeinSi[ii];//质子在硅探测器中射程能损关系
	}
// 	for (i=0;i<1000000;i++)
// 	{
// 		rin=0;
...
// 			z0=pzD2->GetRandom()/1000;
			...
			if(z<z2D1&&z>z1D1)
				cntD1++;//stop in D1
			else if(z>=z2D1)//escape out of D1, towards z+
			{
				...
				if(x<x2D2&&x>x1D2&&y<y2D2&&y>y1D2&&z<z2D2&&z>z1D2)//if stop in D2
					cntD2++;
				else if(z>=z2D2)//if pass through D2, r should be lengthen
				{
			...
			else if(z<=z1D1)//escape out of D1, towards z-, no need to judge whether stop or transmit
			{
				yd=y1QU-y0;//QSDU
