...
#include <time.h>
using namespace std;
void RanD2betaEloss()//能量相关，从D2发射一定能量beta，被各探测器探测到的概率和能损
{
	...
	double rin,Ein,EbetaD1,EbetaD2,EbetaQ1,EbetaQ2,EbetaQL,EbetaQR,EbetaQU,EbetaQD;
	sprintf(anarootname,"V:/RIBLL2015/data24/Si24calabcd_0636_0650Energy.root");
	...
	TH1F *hbetaD1 = new TH1F("hbetaD1","hbetaD1", 1000,0,4000);
	TH1F *hbetaD2 = new TH1F("hbetaD2","hbetaD2", 1000,0,4000);
	TH1F *hbetaQ1 = new TH1F("hbetaQ1","hbetaQ1", 1000,0,4000);
	TH1F *hbetaQ2 = new TH1F("hbetaQ2","hbetaQ2", 1000,0,4000);
	TH1F *hbetaQL = new TH1F("hbetaQL","hbetaQL", 1000,0,4000);
	TH1F *hbetaQR = new TH1F("hbetaQR","hbetaQR", 1000,0,4000);
	TH1F *hbetaQU = new TH1F("hbetaQU","hbetaQU", 1000,0,4000);
	TH1F *hbetaQD = new TH1F("hbetaQD","hbetaQD", 1000,0,4000);
	...
	for(ie=0;ie<1;ie++)
	{
		...
		for (i=0;i<100000;i++)
		{
			...
			if(costheta>0)
			{
				EbetaD2=(z2D2-z0)/costheta*0.37*1000;//3MeV beta in silicon
				hbetaD2->Fill(EbetaD2);
				cntD2++;
				zd=z1Q1-z0;//QSD1
				...
				if(x<x2Q1&&x>x1Q1&&y<y2Q1&&y>y1Q1)//QSD1
				{
					EbetaQ1=(z2Q1-z1Q1)/costheta*0.37*1000;
					cntQ1++;
					if(EbetaQ1>4) hbetaQ1->Fill(EbetaQ1);
				}
				zd=z1Q2-z0;//QSD2
				x=x0+zd*tan(acos(costheta))*cos(phi);//x0+r*sqrt(1-costheta*costheta)*cos(phi);
				y=y0+zd*tan(acos(costheta))*sin(phi);//y0+r*sqrt(1-costheta*costheta)*sin(phi);
				if(x<x2Q2&&x>x1Q2&&y<y2Q2&&y>y1Q2)//QSD2
				{
					EbetaQ2=(z2Q2-z1Q2)/costheta*0.37*1000;
					cntQ2++;
					if(EbetaQ2>0) hbetaQ2->Fill(EbetaQ2);
				}
			}
			if(costheta<0) 
			{
				EbetaD2=(z0-z1D2)/costheta*0.37*1000;
				hbetaD2->Fill(EbetaD2);
				cntD2++;
				zd=z0-z2D1;//DSSD1
				x=x0+zd*tan(acos(costheta))*cos(phi);
				y=y0+zd*tan(acos(costheta))*sin(phi);
				if(x<x2D1&&x>x1D1&&y<y2D1&&y>y1D1)//DSSD1
				{
					EbetaD1=(z1D1-z2D1)/costheta*0.37*1000;
					cntD1++;
					if(EbetaD1>0) hbetaD1->Fill(EbetaD1);
				}
				yd=y1QU-y0;//QSDU
				x=x0+yd/tan(phi);
				z=z0+yd/tan(acos(costheta))/sin(phi);
				if(x<x2QU&&x>x1QU&&z<z2QU&&z>z1QU)//QSDU
				{
					EbetaQU=(y1QU-y2QU)/(tan(acos(costheta))*sin(phi))*0.37*1000;
					cntQU++;
					if(EbetaQU>0) hbetaQU->Fill(EbetaQU);
				}
				yd=y0-y2QD;//QSDD
				x=x0+yd/tan(phi);
				z=z0+yd/tan(acos(costheta))/sin(phi);
				if(x<x2QD&&x>x1QD&&z<z2QD&&z>z1QD)//QSDD
				{
					EbetaQD=(y2QD-y1QD)/(tan(acos(costheta))*sin(phi))*0.37*1000;
					cntQD++; hbetaQD->Fill(EbetaQD);
				}
				xd=x0-x2QL;//QSDL
				y=y0+xd*tan(phi);
				z=z0+xd/tan(acos(costheta))/cos(phi);
				if(y<y2QL&&y>y1QL&&z<z2QL&&z>z1QL)//QSDL
				{
					EbetaQL=(x2QL-x1QL)/(tan(acos(costheta))*cos(phi))*0.37*1000;
					cntQL++; hbetaQL->Fill(EbetaQL);
				}
				xd=x1QR-x0;//QSDR
				y=y0+xd*tan(phi);
				z=z0+xd/tan(acos(costheta))/cos(phi);
				if(y<y2QR&&y>y1QR&&z<z2QR&&z>z1QR)//QSDR
				{
					EbetaQR=(x1QR-x2QR)/(tan(acos(costheta))*cos(phi))*0.37*1000;
					cntQR++; hbetaQR->Fill(EbetaQR);
				}
			}
