...
#include <time.h>
using namespace std;
void RanD2effi27S()//能量相关，从D2发射一定能量质子，被各探测器探测到的概率
{
	...
	// 	}
	sprintf(simurootname,"%s","D:/X/RIBLL2017/S27simulateD2.root");
	...
	ofstream outfile("C:/Si24/Si22peakcali/EffiD2.dat",ios::out);
	...
	for(ie=0;ie<23;ie++)
	{
		...
		for (i=0;i<1000000;i++)
		{
			...
			x0=pxD2->GetRandom();
			y0=pyD2->GetRandom();
			z0=pzD2->GetRandom()/1000+z1D2;
			for(;z0>z2D2;){z0=pzD2->GetRandom()/1000+z1D2;}
			...
			else if(z>=z2D2)//escape out of D2, towards z+, r should be lengthened
			{
				x=x+(z1D3-z2D2)*tan(acos(costheta))*cos(phi);
				y=y+(z1D3-z2D2)*tan(acos(costheta))*sin(phi);
				z=z+(z1D3-z2D2);
				if(x<x2D3&&x>x1D3&&y<y2D3&&y>y1D3&&z<z2D3&&z>z1D3)//if stop in D3
				{
					cntD3++;//stop in D3
				...
				else if(z>=z2D3)//if pass through D3, r should be lengthened
				{
			...
			else if(z<=z1D2)//escape out of D2, towards z-, r should be lengthened
			{
