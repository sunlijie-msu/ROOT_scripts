...
void Si22peakcali()
{
	...
	int IDstart=6,IDstop=8;
	int binmin=400,binmax=1380;
	double sigma=1,thresh=0.16;//sigma should be setted as a small number
	float gaplow=5.,gaphigh=5.;//随分辨不同调整
	unsigned long i;
	int jj,ii;
	...
	//{
		//if()continue;
		sprintf(rawrootname,"V:/RIBLL2015/data24/Si24calabcdMg21_0636_0650.root");
		...
		sprintf(h_name,"%s","h300ALch");
		//cin>>h_name;
		while(strcmp(h_name,"h300ALch")!=0&&strcmp(h_name,"h300BLch")!=0&&strcmp(h_name,"h60ALch")!=0&&strcmp(h_name,"h60BLch")!=0)
		{
...
// 		}//for(i=0;i<nentries;i++) △treeFill
		...
		for(i=IDstart;i<=IDstop;i++)
		{
			...
			canvaspeak[i]=new TCanvas(hfit_name,hfit_name,1000,480);//建立画布
			...
			TPaveText *textgaus = new TPaveText(0.82,0.54,0.99,0.89,"brNDC");//加标注num1左边界，num3右边界，num2下边界，num4上边界
