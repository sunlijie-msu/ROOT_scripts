...
void Si22peakcaliT777()
{
	...
	int IDstart=0,IDstop=2;
	int binmin=300,binmax=4000;
	double sigma=9,thresh=0.03;//sigma should be setted as a small number
	...
	TH1D *histo[20];//TH1F peak search+gauss fit,creat histograms, 
	...
	float chtemp;
	int peaknum;
	const int nummax=6;//search peaknumber can be many, fitnumber can be limited to 5
	const long iprint=100000;//useless for TH1Dget
	const long imax=80000000;//useless for TH1Dget
	double par[10][3];//随峰数不同改动
	//float energy[3]={1320,1862,2036};
	//float energy[3]={5157,5486,5805};
	//float energy[4]={810.6,1680.6,2692.06,5543.36};
	//float energy[4]={964.06,1085.84,1112.09,1408.02};
	float energy[6]={10,20,30,40,50,60};
	...
	//{
		//if()continue;
		//sprintf(rawrootname,"V:/RIBLL2015/data/Si24Clovercali0651.root");
		//sprintf(rawrootname,"V:/RIBLL2015/data24/Si24calabcd0650.root");
		sprintf(rawrootname,"C:/Si24/N1068_test_Rn19.root");//linear cali
		//sprintf(rawrootname,"V:/RIBLL2015/data/QSDcal0005.root");
		...
		cin>>h_name;
// 		while(strcmp(h_name,"h300AL")!=0&&strcmp(h_name,"h300BL")!=0&&strcmp(h_name,"h60AL")!=0&&strcmp(h_name,"h60BL")!=0)
// 		{
...
// 		}//name sequence decide the TH1F sequence in rootfile, name histograms if use Fill(branch) //△treeFill
...
// 		}//for(i=0;i<nentries;i++) △treeFill
		...
		{
			sprintf(histo_name,"%s%d",h_name,ii);
			histo[ii] = (TH1D*)fin->Get(histo_name);
		}//read histograms from a file directly, no need to name histograms, also cannot rebin
		// 	h300AL[0]=(TH1D*)fin->Get("D300ALch0");

		ofstream outfile("C:/Si24/Si22peakcali/Sipeakcali.dat",ios::out);
		for(i=IDstart;i<=IDstop;i++)
		{
			...
			peaknum = s->Search(histo[i],sigma,"",thersh);//△寻峰个数
			...
			for(ii=0;ii<peaknum;ii++)
			{
				outfile<<hfit_name<<" Mean"<<ii<<"=	"<<par[ii][1]<<endl;//输出文本查看
			}
			textgaus->Draw();
			...
			sprintf(paraprint,"slope=%.5f",slope[i]);
			textpol1->AddText(paraprint);
			sprintf(paraprint,"intercept=%.2f",intercept[i]);
			textpol1->AddText(paraprint);
			textpol1->Draw();
			outfile<<hcali_name<<" slope=	"<<slope[i]<<endl;//输出文本查看
			outfile<<hcali_name<<" intercept=	"<<intercept[i]<<endl;//输出文本查看
