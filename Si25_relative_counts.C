...
#include "TLegend.h"
using namespace std;
void Si25_relative_counts()// relative counts in each broadened peaks, goes to Doppler.C for efficiency normalization.
{//also for the ratio of upstream/downstream in peak areas
	time_t tim;
	struct tm *at;
	char now[80];
	int irawroot;
	...
	//{
	//if()continue;
	sprintf(rawrootname,"%s","X:/aaron/rootfiles/run-all_after_cali_25Si.root");//ungated modify
...
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-individual_gated_sun_23Al_include_bad_SeGA_do_not_use_total.root");//gated modify
	...
	{
		if(i==8) continue;
		sprintf(b_name,"%s%02d","SeGA_",i);//ungated modify
//		sprintf(b_name,"%s%d","gbCoincidences",i);//gated modify
	...
	for(i=0;i<=15;i++)//which SeGA detector modify =0<=15
	{
...
// 			maxbinb=histo[i]->FindBin(470);
			...
			minbinb=histo[i]->FindBin(470);
			minbin=histo[i]->FindBin(488);
			maxbin=histo[i]->FindBin(498);
			maxbinb=histo[i]->FindBin(500);
...
// 			maxbinb=histo[i]->FindBin(1400);
			...
			cout<<"SeGA_"<<i<<"	"<<highcounts<<"	"<<lowcounts<<"	"<<peakcounts<<endl;
			//histo[i]->Draw();
