...
#include "TLegend.h"
using namespace std;
void Si25gausnerfcpol1_peakfit_response()// peak draw-fit (gausn+erfc+pol1) used for characterize individual response function for 6 peaks (23Al or 25Si) in each SeGA detector (after gain matching+formal calibration)
{//tau and sigma go to excel, combine 12 tau and sigma for 23Al and 25Si in excel then store them in 12 .dat files then use graphpol12fit_band.C to extract formal parameters to characterize tau and sigma from fitting.
...
// 	ifstream infileupperlimit("D:/X/out/Si25/upperlimitforresponsefunction25Si.dat",ios::in);//12 SeGA detectors
	...
	//{
	//if()continue;
//	sprintf(rawrootname,"%s","X:/aaron/rootfiles/run-all_after_cali_25Si.root");//modify
...
//		histo[i]->Rebin(2);//if couldn't fit correctly, try this
	...
	for(i=1;i<=1;i++)//which SeGA detector modify =0<=15. 0-10 are usually pretty good to fit, 12,13 are troublesome
	{
		if(i==8||i==11||i==14||i==15) continue;
...
// 			if(ii==1){	gaplow=8; gaphigh=7;}//25Si 493
// 			if(ii==2){	gaplow=9; gaphigh=10;}//25Si 945
