...
void analysis_chain_sorted_e21010()// chain sorted runs, old tree is a101 and new tree is a102.
{
	time_t start, tim;
	struct tm* at;
	char now[80];
	...
	sprintf(anarootname, "%s%s%03d%s%03d%s", pathname, "sum_", runstart, "_", runstop, "_sorted.root");//oftenmodify
	...
	TTree* a102 = new TTree("a102", "a102");//or TTree *T888 = new TTree("T888","Treetitle");
	...
	for (int ii = 0; ii < 2; ii++)
	{
		...
		hbG[ii] = new TH1F(h_name, h_name, 4000, 10, 4010);//name sequence decide the TH1F sequence in rootfile
	}
	for (int ii = 0; ii < 2; ii++)
	{
		sprintf(h_name, "%s%d", "hpG", ii);
		hpG[ii] = new TH1F(h_name, h_name, 4000, 10, 4010);//name sequence decide the TH1F sequence in rootfile
	}
	...
	for (i = 0; i < totalentries; i++)
	{
		...
		if (mul > 0 && mul < 66)
			...
			{// find which FE[ii] is Ge based on id[ii]
				if (FE[ii] > 0 && id[ii] == 12)
				{
					FE[ii] = (FE[ii] + 78.08) / 0.8284;
					FE[ii] = FE[ii] * 0.82741082 - 77.642761;// calibration based on 152Eu, 226Ra, and room background
					hG[0]->Fill(FE[ii]);
				}
				if (FE[ii] > 0 && id[ii] == 13)
				{
					FE[ii] = (FE[ii] + 76.372) / 0.79083;
					FE[ii] = FE[ii] * 0.79025883 - 76.111357;// calibration based on 152Eu, 226Ra, and room background
					hG[1]->Fill(FE[ii]);
				}
