...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit()// peak draw-fit (gausn+pol1)
{
	time_t tim;
	struct tm *at;
	char now[80];
	int irawroot;
	char rawrootname[100];
	double slope[300];
	double intercept[300];
	const int ID1=1;//adjust
	const int ID2=2;//adjust
	const int binmin=50,binmax=400;//adjust//search for peaks in this range
	const int binwidth=1;
	int binnum=(binmax-binmin)/binwidth;
	double sigma=2,thresh=0.5,sigmab=20;//adjust
	float gaplow=25.,gaphigh=25.;//fitting range随分辨不同调整//adjust
	unsigned long i;
	int jj,ii;
	...
	//{
	//if()continue;
//	sprintf(rawrootname,"X:/T777/Rn559_152Eu_DSSD3.root");//modify if change path
	sprintf(rawrootname,"X:/T777/copysmall.root");//modify if change path
	...
	unsigned long nentries=T111->GetEntries();//读事件数
	cout<<rawrootname;
...
// 	}
	...
	for(ii=ID1;ii<ID2;ii++)
	{
		sprintf(command1,"%s%s%d%s%s",b_name,"[",ii,"]>>",histo_name);
		//sprintf(command1,"%s%s%d%s%f%s%s",b_name,"[",ii,"]+",gRandom->Uniform(-1,1),">>",histo_name);//useless only one random number was generated
		puts(command1);
		sprintf(command2,"%s%s%d%s%d",b_name,"[",ii,"]>",binmin);
		puts(command2);
		T111->Draw(command1,command2);//tree-Draw("DSSD1AL[0]>>hDSSD1AL[0]")
	}
// 	{
...
// 	}
	...
	ofstream outfile("D:/X/out/peakcali.dat",ios::out);//modify if change path
	for(i=ID1;i<ID2;i++)
	{
		...
		//for(ii=0;ii<peaknum;ii++)
		{
			total[ii]=new TF1("total","[0]*x+[1]+[2]/2/[3]*exp(0.5*([4]*[4]/([3]*[3]))-(x-[5])/[3])*ROOT::Math::erfc(1/sqrt(2)*([4]/[3]-(x-[5])/[4]))",peakx[ii]-gaplow,peakx[ii]+gaphigh);//高斯拟合，调节合适的拟合range / Glassman
 			g[ii]=new TF1("g","gausn",peakx[ii]-gaplow,peakx[ii]+gaphigh);// The [2]-N parameter in total is equivalent to the Constant in gausn
 			p[ii]=new TF1("p","([0]*x+[1]+[2]/2/[3])/([0]*x+[1]+[2]/2/[3])*exp(0.5*([4]*[4]/([3]*[3]))-(x-[5])/[3])*ROOT::Math::erfc(1/sqrt(2)*([4]/[3]-(x-[5])/[4]))",peakx[ii]-gaplow,peakx[ii]+gaphigh);

// 			total[ii]=new TF1("total","0*([0]*x+[1])+[2]/2/[3]*exp([4]/[3]*[4]/[3]/2+(x-[5])/[3])*(1-ROOT::Math::erf([4]*[4]+[3]*(x-[5])/sqrt(2)/[4]/[3]))",peakx[ii]-gaplow,peakx[ii]+gaphigh);//高斯拟合，调节合适的拟合range / Bennett
// 			g[ii]=new TF1("g","[0]*exp(-0.5*((x-[1])/[2])*((x-[1])/[2]))",peakx[ii]-gaplow,peakx[ii]+gaphigh);
// 			p[ii]=new TF1("p","0*([0]*x+[1])+[2]/2/[3]*exp(0.5*([4]*[4]/([3]*[3]))-(x-[5])/[3])",peakx[ii]-gaplow,peakx[ii]+gaphigh);
...
// 			total[ii]->SetParameters(0.1,15,30000,20,6.6,418);//initial value [0]-A, [1]-B, [2]-N, [3]-λ, [4]-σ, [5]-μ
// 			total[ii]->SetParLimits(0,-10,10);//Bkg A
// 			total[ii]->SetParLimits(1,-400,400);//Bkg B
// 			total[ii]->SetParLimits(2,18000,1000000);//Constant,min,max
// 			total[ii]->SetParLimits(3,0.1,50);//Lambda
// 			total[ii]->SetParLimits(4,1,10);//Sigma
// 			total[ii]->SetParLimits(5,400,420);//Mean

			//total[ii]->SetParameters(0.1,15,peaky[ii],10,10,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-λ, [4]-σ, [5]-μ
			total[ii]->SetParameters(2,500,60000,0.2,3,120);//initial value [0]-A, [1]-B, [2]-N, [3]-λ, [4]-σ, [5]-μ
			total[ii]->SetParLimits(0,1,100);//Bkg A
			total[ii]->SetParLimits(1,-5000,5000);//Bkg B
			total[ii]->SetParLimits(2,10000,100000);//Constant,min,max
			total[ii]->SetParLimits(3,0,5);//Lambda
			total[ii]->SetParLimits(4,1,4);//Sigma
			total[ii]->SetParLimits(5,100,130);//Mean
			...
			g[ii]->SetParameters(par[ii][2],par[ii][5],par[ii][4]);//set parameters for drawing
			g[ii]->SetLineColor(4);
			p[ii]->SetParameters(par[ii][0],par[ii][1],par[ii][2],par[ii][3],par[ii][4],par[ii][5]);//set parameters for drawing
			p[ii]->SetLineColor(8);
			g[ii]->Draw("same");
			p[ii]->Draw("same");
			histo[i]->GetXaxis()->SetRangeUser(peakx[ii]-2*gaplow,peakx[ii]+2*gaphigh);//zoom the axis
		}
		...
		//for(ii=0;ii<peaknum;ii++)
		{
			...
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Sigma%d=%.1f%s%.3f",ii,par[ii][4],"+/-",parerr[4]);
			textgaus->AddText(paraprint);
		...
		sprintf(hfit_name,"%s%s%s","D:/X/out/",hfit_name,".png");//modify if change path
