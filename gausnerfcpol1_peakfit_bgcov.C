...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bgcov()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed γ rays to get the energy and intesity.
{//for 25Si known bg
...
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-gated_sun_25Si_10peakscali.root");
	...
	for(i=ID1;i<=ID2;i++)//which detector
	{
		histo[i]= (TH1F*)fin->Get("SeGA_00"); //Get Gamma ray spectrum ungated modify
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		histo[i]->GetYaxis()->SetTitle("Counts per 0.1 keV");//轴名
...
// 			if(ii==5){	gaplow=25; gaphigh=25;}//23Al 7803
 			if(ii==0){	gaplow=5; gaphigh=4;}//25Si 452 old bg modify
 			if(ii==1){	gaplow=6; gaphigh=5;}//25Si 493
 			if(ii==2){	gaplow=6; gaphigh=5;}//25Si 945
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParameters(aguess,bguess,peaky[ii],0.9,sigmaguess,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
			...
			total[ii]->SetParLimits(2,1,100000);//Constant,min,max  //Glassman
			total[ii]->SetParLimits(3,0.0001,5);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,0.2,9);//Sigma  //Glassman
			total[ii]->SetParLimits(5,480,520);//Mean  //Glassman
...
//			TFitResultPtr r_fit = histo[i]->Fit("total","MLES","",peakx[ii]-gaplow,peakx[ii]+gaphigh);//"S" means the result of the fit is returned in the TFitResultPtr
			...
			TFitResultPtr r_sig = histo[i]->Fit("total","MLES","",peakx[ii]-gaplow,peakx[ii]+gaphigh);///specify a range in the Fit
...
//			hint_sig->Draw("e3 same");//draw the confidence band [colored] showing 1 or 2 or 3 standard deviation uncertainty
		...
		cout<<sqrt(Utau_Utau+Umean_Umean+2*rou_Utau_Umean)<<endl;
			...
			sprintf(paraprint,"Constant/binsize%d=%.2f%s%.3f",ii,par[ii][2]/binwidth,"+/-",parerr[ii][2]/binwidth);//par数组还保持着刚才的参数
