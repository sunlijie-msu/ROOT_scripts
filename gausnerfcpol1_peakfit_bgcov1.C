...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bgcov1()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed γ rays to get the energy and intesity.
{//for new 25Si bg
	...
	const int nummax=10;//search peaknumber can be many, fitnumber can be limited to 3//no need to modify
	int peaknum=nummax;
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
		histo[i]->Rebin(10);//rebin 10 will get 1-keV bin
	...
	{//don't change i=0 to other values
		...
		for(ii=4; ii<=4; ii++)//which peak in one SeGA detector modify =0<=5
		{
			//if(ii==4)continue;
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParLimits(5,830,8000);//Mean  //Glassman
