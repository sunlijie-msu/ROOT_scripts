...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bgcov23Al()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed γ rays to get the energy and intesity.
{//for 25Si known bg
...
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-gated_sun_25Si.root");//modify
	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-ungated_sun_23Al.root");//modify formal Eg
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=5; ii<=5; ii++)//which peak in one SeGA detector modify =0<=5
		{
			//if(ii==4)continue;
			...
			if(ii==2){	gaplow=6; gaphigh=7;}//23Al 1600
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParLimits(5,7800,8000);//Mean  //Glassman
