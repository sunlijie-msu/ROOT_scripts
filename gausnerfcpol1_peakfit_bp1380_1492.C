...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bp1380_1492()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed proton to get the energy and intensity.
{//for 25Si known bp
...
// 	ifstream infileupperlimit("D:/X/out/Si25/upperlimitforresponsefunction25Si_newbg.dat",ios::in);//12 SeGA detectors
	...
	for(i=ID1;i<=ID2;i++)//which detector
	{
		...
		for(ii=0;ii<nummax;ii++)//which peak
		{
			...
			lowerlimit[i][ii]=1310;
			upperlimit[i][ii]=1520;
		}
	}
	for(i=ID1;i<=ID2;i++)//which detector
	{
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
		histo[i]->Rebin(2);//rebin 10 will get 1-keV bin
	...
	{//don't change i=0 to other values
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			if(ii==0){	gaplow=60; gaphigh=70; peakdiff1=0; peakdiff2=100;}//25Si 452 old bg modify peakdiff1=0 if peak1 is bigger; peakdiff2=0 if peak2 is bigger
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParameters(aguess,bguess,peaky[ii],2,sigmaguess,peakx[ii]-peakdiff1, peaky[ii],2,sigmaguess,peakx[ii]+peakdiff2);//initial value [0]-A, [1]-B, [2]-N1, [3]-τ1, [4]-σ1, [5]-μ1, [6]-N2, [7]-τ2, [8]-σ2, [9]-μ2
			...
			total[ii]->SetParLimits(5,1370,1390);//Mean  //Glassman
			...
			total[ii]->SetParLimits(9,1470,1510);//Mean  //Glassman
