...
{//for 25Si known bp
	...
	float gaplow=70.,gaphigh=70., peakdiff1=100., peakdiff2=0.;//fitting range随分辨不同调整
	unsigned long i;
...
// 	ifstream infileupperlimit("D:/X/out/Si25/upperlimitforresponsefunction25Si_newbg.dat",ios::in);//12 SeGA detectors
	...
	for(i=ID1;i<=ID2;i++)//which detector
	{
		...
		for(ii=0;ii<nummax;ii++)//which peak
		{
			...
			lowerlimit[i][ii]=1700;
			upperlimit[i][ii]=2070;
		}
	}
	for(i=ID1;i<=ID2;i++)//which detector
	{
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			if(ii==0){	gaplow=100; gaphigh=130; peakdiff1=100; peakdiff2=0;}//25Si 452 old bg modify peakdiff1=0 if peak1 is bigger; peakdiff2=0 if peak2 is bigger
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParLimits(3,0.3,85);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,2,29);//Sigma  //Glassman
			total[ii]->SetParLimits(5,1760,1860);//Mean  //Glassman
			total[ii]->SetParLimits(6,1,5000000);//Constant,min,max  //Glassman
			total[ii]->SetParLimits(7,0.3,85);//Tau  //Glassman portal
			total[ii]->SetParLimits(8,2,39);//Sigma  //Glassman
			total[ii]->SetParLimits(9,1860,1940);//Mean  //Glassman
