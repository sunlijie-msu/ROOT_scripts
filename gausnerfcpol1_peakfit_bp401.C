...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bp()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed proton to get the energy and intensity.
{//for 25Si known bp
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			if(ii==0){	gaplow=80; gaphigh=95;}//25Si 452 old bg modify
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParameters(aguess,bguess,peaky[ii],2.2,sigmaguess,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
			...
			total[ii]->SetParLimits(3,0.0001,35);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,2,39);//Sigma  //Glassman
			total[ii]->SetParLimits(5,360,450);//Mean  //Glassman
