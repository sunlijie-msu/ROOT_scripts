...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			if(ii==0){	gaplow=93; gaphigh=76;}//25Si 452 old bg modify
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParameters(aguess,bguess,peaky[ii],35,sigmaguess,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
			...
			total[ii]->SetParLimits(3,3,65);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,12,89);//Sigma  //Glassman
			total[ii]->SetParLimits(5,670,750);//Mean  //Glassman
