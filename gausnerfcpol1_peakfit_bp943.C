...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bp943()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed proton to get the energy and intensity.
{//for 25Si known bp
...
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-gated_sun_25Si_10peakscali.root");
	...
	for(i=ID1;i<=ID2;i++)//which detector
	{
		histo[i]= (TH1F*)fin->Get("h6"); //Get Gamma ray spectrum ungated modify
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		if(i==8||i==11||i==14||i==15) continue;
		sprintf(hfit_name,"%s%d","fit_proton_5pads_",i);
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			if(ii==0){	gaplow=95; gaphigh=88;}//25Si 452 old bg modify
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParameters(aguess,bguess,peaky[ii],2,sigmaguess,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
			...
			total[ii]->SetParLimits(3,0.3,65);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,2,29);//Sigma  //Glassman
			total[ii]->SetParLimits(5,900,960);//Mean  //Glassman
