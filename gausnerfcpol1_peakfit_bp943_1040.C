...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bp943_1040()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed proton to get the energy and intensity.
{//for 25Si known bp
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			if(ii==0){	gaplow=100; gaphigh=100; peakdiff1=0; peakdiff2=70;}//25Si 452 old bg modify peakdiff1=0 if peak1 is bigger; peakdiff2=0 if peak2 is bigger
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParLimits(5,910,980);//Mean  //Glassman
			...
			total[ii]->SetParLimits(9,1000,1050);//Mean  //Glassman
