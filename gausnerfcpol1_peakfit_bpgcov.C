...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_bpgcov()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed γ rays to get the energy and intesity.
{//for 25Si bpg without Doppler, how bad the fit will be?
...
//	double energyliterr[6]={0.137758, 0.005000, 0.365148, 0.010000, 0.603657, 0.485071};//23Al
	...
	double parchi[nummax],parNDF[nummax],p_value[nummax];
 	ifstream infilelowerlimit("D:/X/out/Si25/lowerlimitforresponsefunction25Si_bpg.dat",ios::in);//12 SeGA detectors // modify
 	ifstream infileupperlimit("D:/X/out/Si25/upperlimitforresponsefunction25Si_bpg.dat",ios::in);//12 SeGA detectors
...
//		histo[i]= (TH1F*)fin->Get("SeGA_00"); //Get Gamma ray spectrum ungated modify
	...
	{//don't change i=0 to other values
		...
		for(ii=0; ii<=0; ii++)//which peak in one SeGA detector modify =0<=5
		{
			...
			peakx[0]=1369; peakx[1]=2754; peakx[2]=2870; peakx[3]=4238;
...
// 			if(ii==5){	gaplow=25; gaphigh=25;}//23Al 7803
 			if(ii==0){	gaplow=25; gaphigh=25;}//25Si 452 old bg modify
 			if(ii==1){	gaplow=30; gaphigh=30;}//25Si 493
 			if(ii==2){	gaplow=30; gaphigh=30;}//25Si 945
 			if(ii==3){	gaplow=40; gaphigh=40;}//25Si 1461
...
// 			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParLimits(2,1,1000000);//Constant,min,max  //Glassman
			total[ii]->SetParLimits(3,0.0001,15);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,0.2,29);//Sigma  //Glassman
			total[ii]->SetParLimits(5,430,4300);//Mean  //Glassman
