...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_cov_DSL2()// peak draw-fit (gausn+erfc+pol1) used for fit one peak, used to formally fit β-delayed γ rays to get the energy and intesity.
{//for shoulder gaussian
	...
	TF1 *p[nummax], *g[nummax], *b[nummax], *p2[nummax];
...
//	double energyliterr[6]={0.137758, 0.005000, 0.365148, 0.010000, 0.603657, 0.485071};//23Al
	float peakmin;
	float chtemp;
	double par[nummax][6],parerr[nummax][6];//随峰数不同改动par[peaknum][3]//adjust
	double parchi[nummax],parNDF[nummax],p_value[nummax];
 	ifstream infilelowerlimit("/mnt/analysis/triumf_s2193/sun/Histograms/lower_limit_for_uncalibrated_alpha_source.dat",ios::in);//12 SeGA detectors // modify
 	ifstream infileupperlimit("/mnt/analysis/triumf_s2193/sun/Histograms/upper_limit_for_uncalibrated_alpha_source.dat",ios::in);//12 SeGA detectors
// 	ifstream infilelowerlimit("D:/X/out/Si25/lowerlimitforresponsefunction25Si_newbg.dat",ios::in);//12 SeGA detectors // modify
// 	ifstream infileupperlimit("D:/X/out/Si25/upperlimitforresponsefunction25Si_newbg.dat",ios::in);//12 SeGA detectors
	...
	//cout<<"input runstop:";
	//cin>>runstop;
	ofstream outfile("/mnt/analysis/triumf_s2193/sun/Histograms/peakcali.dat",ios::out);
	ofstream outfile2("/mnt/analysis/triumf_s2193/sun/Histograms/peakcalipara.dat",ios::out);
	//FILE *outfile=fopen ("D:/X/peakcali.dat","a");
	//for(irawroot=runstart; irawroot<=runstop; irawroot++)
	//{
	//if()continue;
	sprintf(rawrootname,"%s","/mnt/analysis/triumf_s2193/lexie/Histograms/Sum-tree_00370.root");//modify formal Eg
...
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-gated_sun_25Si_10peakscali.root");
	...
	for(i=ID1;i<=ID2;i++)//which detector
	{
		histo[i]= (TH1F*)fin->Get("h3"); //Get Gamma ray spectrum ungated modify
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
		histo[i]->Rebin(1);//rebin 10 will get 1-keV bin
	...
	{//don't change i=0 to other values
		if(i==8||i==11||i==14||i==15) continue;
		sprintf(hfit_name,"%s%d","fit_Rn370_",i);
		canvaspeak[i]=new TCanvas(hfit_name,hfit_name,1200,600);//建立画布
		canvaspeak[i]->cd();//进入画布
		canvaspeak[i]->SetTopMargin(0.02);
		canvaspeak[i]->SetRightMargin(0.03);
		canvaspeak[i]->SetLeftMargin(0.11);
		canvaspeak[i]->SetBottomMargin(0.12);
		...
		for(ii=1; ii<=1; ii++)//which peak in one SeGA detector modify =0<=5
		{
			//if(ii==4)continue;
...
// 			if(ii==5){	gaplow=25; gaphigh=25;}//23Al 7803
 			if(ii==0){	gaplow=70; gaphigh=40;}//5155
 			if(ii==1){	gaplow=50; gaphigh=50;}//5486
 			if(ii==2){	gaplow=50; gaphigh=50;}//5805
 			if(ii==3){	gaplow=10; gaphigh=10;}//25Si 1461
 			if(ii==4){	gaplow=8; gaphigh=7;}//25Si 1612
 			if(ii==5){	gaplow=14; gaphigh=14;}//25Si 2614
// 			if(ii==0){	gaplow=7; gaphigh=8;}//25Si 845 new bg modify
...
// 			if(ii==9){	gaplow=44; gaphigh=44;}//25Si 7900-511*2
			...
			//total[ii]=new TF1("total","[0]*x+[1]+[2]/2/[3]*exp(0.5*([4]*[4]/([3]*[3]))+(x-[5])/[3])*ROOT::Math::erfc(1/sqrt(2)*([4]/[3]+(x-[5])/[4]))",peakx[ii]-gaplow,peakx[ii]+gaphigh);// Glassman PLB2018 N=area low-energy tail
			total[ii]=new TF1("total","[0]*x+[1]+[2]/2/[3]*exp(0.5*([4]*[4]/([3]*[3]))+(x-[5])/[3])*ROOT::Math::erfc(1/sqrt(2)*([4]/[3]+(x-[5])/[4]))", peakx[ii] - gaplow, peakx[ii] + gaphigh);
...
// 			p2[ii]->SetNpx((gaphigh + gaplow) * 10);
			...
			total[ii]->SetParameters(aguess,bguess,peaky[ii],0.9,sigmaguess,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ, [6]-N2, [7]-μ2, [8]-σ2
			...
			total[ii]->SetParLimits(2,100,2000000);//Constant,min,max  //Glassman
			total[ii]->SetParLimits(3,0.0001,50);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,0.2,90);//Sigma  //Glassman
			total[ii]->SetParLimits(5,1700,2100);//Mean  //Glassman
// 			total[ii]->SetParLimits(6,0,2000000);//Constant2
// 			total[ii]->SetParLimits(7,1893,1896);//Mean2
// 			total[ii]->SetParLimits(8,0.2,7);//Sigma2
...
//			TFitResultPtr r_fit = histo[i]->Fit("total","MLES","",peakx[ii]-gaplow,peakx[ii]+gaphigh);//"S" means the result of the fit is returned in the TFitResultPtr
			...
			//histo[i]->Fit("total","MLES","",peakx[ii]-gaplow,peakx[ii]+gaphigh);///specify a range in the Fit
			TFitResultPtr r_sig = histo[i]->Fit("total","MLES","",peakx[ii]-10,peakx[ii]+gaphigh);///specify a range in the Fit
			...
			p_value[ii]=total[ii]->GetProb();//This probability is not the “probability that your fit is good.” If you did many fake experiments (draw many random samples of data points from the assumed distribution (your fit function)), this is the percentage of experiments that would give χ2 values ≥ to the one you got in this experiment.
...
// 			p2[ii]->SetLineColor(4);
			...
			b[ii]->SetLineColor(8);
			total[ii]->SetLineWidth(3);
			// Maybe EMG is too complicated, the confidence band is extremely wide or inaccurate to show.
			//If the option "S" is instead used, TFitResultPtr contains the TFitResult and behaves as a smart pointer to it.
			//The fit parameters, error and chi2 (but not covariance matrix) can be retrieved also from the fitted function.
...
// 			p2[ii]->Draw("same");
		...
		cout<<sqrt(Utau_Utau+Umean_Umean+2*rou_Utau_Umean)<<endl;
			...
			sprintf(paraprint,"Mean%d=%.4f%s%.4f",ii,par[ii][5],"+/-",parerr[ii][5]);
			textgaus->AddText(paraprint);
			...
			sprintf(h_name,"/mnt/analysis/triumf_s2193/sun/Histograms/%s%s%d_%s.png",hfit_name,"_peak",ii,tauflag);
