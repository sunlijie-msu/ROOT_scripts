...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_response_DSL()// peak draw-fit (gausn+erfc+pol1) used for characterize individual response function for 6 peaks in the 0 degree GRIFFIN detector (after Cathleen's calibration)
{//tau and sigma go to excel, then store them in .dat files then use graphpol12fit_band_sigmatau_DSL.C to extract formal parameters to characterize tau and sigma from fitting.
	...
	double energylit[6]={279.01, 547.5, 2814.06, 3597.26, 3736.5, 6128.63};//31S
	double energyliterr[6]={0.05, 0.3, 0.2, 0.25, 0.3, 0.04};//31S DSLmodify
	float peakmin;
	float chtemp;
...
//		sprintf(b_name,"hGriff_0");//Get Gamma ray spectrum for DSL 0 deg GRIFFIN
		sprintf(b_name,"hGriffinAddback_0");//Get Gamma ray spectrum for DSL 0 deg GRIFFIN response function characterization
		//up or down?
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=5; ii<=5; ii++)//which peak in one SeGA detector modify =0<=5//DSLmodify
		{
			//if(ii==4)continue;
			...
			if(ii==5){	gaplow=30; gaphigh=20;}//16O 6129
...
//			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParLimits(2,10,500000000);//Constant,min,max  //Glassman
			total[ii]->SetParLimits(3,-10,10);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,0,6);//Sigma  //Glassman
			total[ii]->SetParLimits(5,6125,6133);//Mean  //Glassman//DSLmodify
