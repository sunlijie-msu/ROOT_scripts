...
#include "TLegend.h"
using namespace std;
void gausnerfcpol1_peakfit_response_DSL_4156()// peak draw-fit (gausn+erfc+pol1) used for characterize individual response function for 6 peaks in the 0 degree GRIFFIN detector (after Cathleen's calibration)
{//tau and sigma go to excel, then store them in .dat files then use graphpol12fit_band_sigmatau_DSL.C to extract formal parameters to characterize tau and sigma from fitting.
	...
	const int nummax=7;//search peaknumber can be many, fitnumber can be limited to 3//no need to modify
	int peaknum=nummax;
	...
	double energylit[7]={279.01, 547.5, 2814.06, 3597.26, 3736.5, 6128.63, 4470};//31S
	double energyliterr[7]={0.05, 0.3, 0.2, 0.25, 0.3, 0.04, 3};//31S DSLmodify
	float peakmin;
	float chtemp;
...
//			infileupperlimit>>upperlimit[i][ii];
			lowerlimit[i][ii]=energylit[ii]-20;
			upperlimit[i][ii]=energylit[ii]+20;
		}
	}
	for(i=ID1;i<=ID2;i++)//which detector
	{
		...
		for(ii=0;ii<nummax;ii++)//which peak
		{
			cout<<"	L="<<lowerlimit[i][ii]<<"	U="<<upperlimit[i][ii];
		}
		cout<<endl;
	}
	...
	//{
	//if()continue;
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-ungated_sun_25Si.root");//
	sprintf(rawrootname,"%s","D:/X/out/DSL/moreAlphaEgates.root");//DSLmodify
	//up or down?
// 	sprintf(rawrootname,"%s","D:/X/out/DSL/K39_Eg2814_tauinfinity_EMG.root");
//  sprintf(rawrootname,"%s","D:/X/out/DSL/O16_Eg6129_tauinfinity_EMG.root");
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-gated_sun_25Si.root");//
...
//	sprintf(rawrootname,"%s","D:/X/out/Si25/100eV-bin-gated_sun_25Si_10peakscali.root");
	...
	for(i=ID1;i<=ID2;i++)//which detector
	{
//		sprintf(b_name,"hGriff_0");//Get Gamma ray spectrum for DSL 0 deg GRIFFIN
		sprintf(b_name,"hGriffinAddback_alpha39_t0");//Get Gamma ray spectrum for DSL 0 deg GRIFFIN response function characterization
		//up or down?
//		sprintf(b_name,"Eg");//Simulated Gamma ray spectrum//DSLmodify
		histo[i] = (TH1F*)fin->Get(b_name);
		histo[i]->SetLineColor(1);
//		histo[i]= (TH1F*)fin->Get("h6"); //Get Gamma ray spectrum ungated 
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
		histo[i]->Rebin(1);//rebin 10 will get 10-keV bin
	...
	{//don't change i=0 to other values
		if(i==8||i==11||i==14||i==15) continue;
		sprintf(hfit_name,"%s%d","GRIFFIN deg",i);
		...
		for(ii=6; ii<=6; ii++)//which peak in one SeGA detector modify =0<=5//DSLmodify
		{
			//if(ii==4)continue;
			...
			if(ii==0){	gaplow=8; gaphigh=10;}//197Au 279 modify
			if(ii==1){	gaplow=10; gaphigh=10;}//197Au 547
			if(ii==2){	gaplow=18; gaphigh=10;}//39K 2814
			if(ii==3){	gaplow=20; gaphigh=12;}//39K 3597
			if(ii==4){	gaplow=19; gaphigh=12; }//39K 3737
			if(ii==5){	gaplow=32; gaphigh=18;}//16O 6129
			if(ii==6){	gaplow=50; gaphigh=46;}//31S 4156
//  			if(ii==0){	gaplow=8; gaphigh=8;}//25Si 452 old bg
//  			if(ii==1){	gaplow=8; gaphigh=8;}//25Si 493
//  			if(ii==2){	gaplow=9; gaphigh=8;}//25Si 945
//  			if(ii==3){	gaplow=10; gaphigh=10;}//25Si 1461
//  			if(ii==4){	gaplow=11; gaphigh=8;}//25Si 1612
//  			if(ii==5){	gaplow=14; gaphigh=14;}//25Si 2614
// 			if(ii==0){	gaplow=7; gaphigh=8;}//25Si 845 new bg
// 			if(ii==1){	gaplow=5; gaphigh=8;}//25Si 1338
// 			if(ii==2){	gaplow=9; gaphigh=9;}//25Si 1789
// 			if(ii==3){	gaplow=7; gaphigh=6;}//25Si 884
// 			if(ii==5){	gaplow=9; gaphigh=13;}//25Si 2222
// 			if(ii==6){	gaplow=13; gaphigh=11;}//25Si 2673
// 			if(ii==7){	gaplow=28; gaphigh=26;}//25Si 6288
// 			if(ii==8){	gaplow=38; gaphigh=40;}//25Si 7900
// 			if(ii==4){	gaplow=40; gaphigh=44;}//25Si 7900-511
// 			if(ii==9){	gaplow=44; gaphigh=44;}//25Si 7900-511*2
			...
			highcounts=highcounts/8; lowcounts=lowcounts/8;
...
//			total[ii]=new TF1("total","[0]*x+[1]+[2]/2/[3]*exp(0.5*([4]*[4]/([3]*[3]))-(x-[5])/[3])*ROOT::Math::erfc(1/sqrt(2)*([4]/[3]-(x-[5])/[4]))",peakx[ii]-gaplow,peakx[ii]+gaphigh);// Glassman PLB2018 N=area high-energy tail
...
//			b[ii]=new TF1("b","[0]*x+[1]",peakx[ii]-gaplow,peakx[ii]+gaphigh);//pure bkg
			...
			total[ii]->SetParameters(aguess/2,bguess/2,peaky[ii],0.7,sigmaguess,peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
			...
			//total[ii]->SetParLimits(2,0,50000000);//Constant,min,max  //Glassman
			total[ii]->SetParLimits(3,0,20);//Tau  //Glassman portal
			//			total[ii]->SetParLimits(3,0.7,0.7);//Tau  //Glassman portal
			total[ii]->SetParLimits(4,1.5,1.7);//Sigma  //Glassman
			total[ii]->SetParLimits(5,4460,4480);//Mean  //Glassman//DSLmodify
...
//			TFitResultPtr r_fit = histo[i]->Fit("total","MLES","",peakx[ii]-gaplow,peakx[ii]+gaphigh);//"S" means the result of the fit is returned in the TFitResultPtr
			...
			histo[i]->Fit("total","MLE","",peakx[ii]-gaplow,peakx[ii]+gaphigh);///specify a range in the Fit
			histo[i]->Fit("total","L","",peakx[ii]-gaplow,peakx[ii]+gaphigh);///specify a range in the Fit
...
//		cor.Print();
		...
		cout<<sqrt(Utau_Utau+Umean_Umean+2*rou_Utau_Umean)<<endl;
			...
			peakxerr[ii]=sqrt(Utau_Utau+Umean_Umean+2*rou_Utau_Umean);//a proxy/substitute for assessing the error of Maximum
			...
			outfile<<"SeGA_"<<i<<"	Constant*bin"<<ii<<"=	"<<par[ii][2]<<"	+/-	"<<parerr[ii][2]<<"	Mean"<<ii<<"=	"<<par[ii][5]<<"	+/-	"<<parerr[ii][5]<<"	Maximum"<<ii<<"=	"<<peakx[ii]<<"	+/-	"<<setprecision(13)<<peakxerr[ii]<<"	Sigma"<<ii<<"=	"<<par[ii][4]<<"	+/-	"<<parerr[ii][4]<<"	Tau"<<ii<<"=	"<<par[ii][3]<<"	+/-	"<<parerr[ii][3]<<"	A"<<ii<<"=	"<<par[ii][0]<<"	+/-	"<<parerr[ii][0]<<"	B"<<ii<<"=	"<<par[ii][1]<<"	+/-	"<<parerr[ii][1]<<"	Chi2"<<ii<<"=	"<<parchi[ii]<<"	NDF"<<ii<<"=	"<<parNDF[ii]<<"	Area"<<ii<<"=	"<<sqrt(3.1415926536*2)*par[ii][2]*par[ii][4]<<endl;//输出文本查看
			...
			sprintf(paraprint,"Constant*bin%d=%.2f%s%.3f",ii,par[ii][2]/binwidth,"+/-",parerr[ii][2]/binwidth);//par数组还保持着刚才的参数
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Mean%d=%.6f%s%.6f",ii,par[ii][5],"+/-",parerr[ii][5]);
			textgaus->AddText(paraprint);
			...
			textgaus->AddText(paraprint);
			sprintf(paraprint,"p-val%d=%.5f",ii,p_value[ii]);
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Maximum%d=%.6f%s%.6f",ii,peakx[ii],"+/-",peakxerr[ii]);//par数组还保持着刚才的参数
			textgaus->AddText(paraprint);
			textgaus->Draw();
			//sprintf(h_name,"D:/X/out/response23Al/%s%s%d_%s.png",hfit_name,"_peak",ii,tauflag);
			sprintf(h_name,"D:/X/out/Figure_DSL/%s%s%d_%s.png",hfit_name,"_peak",ii,tauflag);//modify
