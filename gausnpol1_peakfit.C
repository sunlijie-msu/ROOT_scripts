...
#include "TLegend.h"
using namespace std;
void gausnpol1_peakfit()// peak draw-fit (gausn+pol1) used for 25Si SeGA gain matching by runs
{
	time_t tim;
	struct tm *at;
	char now[80];
	...
	const int ID2=15;//adjust i<=ID2
	const int binmin=3000,binmax=6100;//adjust//search for peaks in this range 511
	const int binmin=9000,binmax=16500;//adjust//search for peaks in this range 1461
	const int binmin=17500,binmax=29900;//adjust//search for peaks in this range 2614
	const int binwidth=1;
	int binnum=(binmax-binmin)/binwidth;
	double sigma=1,thresh=0.6,sigmab=20;//adjust
	float gaplow=105.,gaphigh=105.;//fitting range随分辨不同调整//adjust
	unsigned long i;
	int jj,ii;
	char paraprint[100],b_name[50],histo_name[50],h_name[50],hfit_name[50],hcali_name[50];
	...
	int runstart=123,runstop=140;//modify	
// 	cout<<"input runstart:";
// 	cin>>runstart;
	cout<<"input runstop:";
	cin>>runstop;
	runstart=runstop;
	ofstream outfile("D:/X/peakcali.dat",ios::app);//modify if change path
	...
	//sprintf(rawrootname,"D:/X/ExG4.root");
	sprintf(rawrootname,"%s%04d%s","X:/aaron/rootfiles/run-",irawroot,"-all.root");//modify if change path
	...
	unsigned long nentries=outTree->GetEntries();//读事件数
	cout<<rawrootname;
...
// 	}
	...
	{
		if(i==8) continue;
		sprintf(hfit_name,"%s%d%s%d","Rn",irawroot,"_SeGA_",i);
...
// 		}
		...
		for(ii=0;ii<peaknum;ii++)
		{
			...
			total[ii]->SetParameters(peaky[ii],peakx[ii],10,0,0);//initial value
			total[ii]->SetParLimits(0,60,1000000);//Constant,min,max
			total[ii]->SetParLimits(1,peakx[ii]-100,peakx[ii]+100);//Mean
			total[ii]->SetParLimits(2,1,50);//Sigma
			//total[ii]->SetParLimits(3,-1000,-10);//A
			//total[ii]->SetParLimits(4,0,0);//B
			total[ii]->SetParNames("Const","Mean","Sigma","A","B");
			histo[i]->Fit("total","M","",peakx[ii]-gaplow,peakx[ii]+gaphigh);///specify a range in the Fit
		...
		TPaveText *textgaus = new TPaveText(0.73,0.15,0.99,0.77,"brNDC");//加标注left, down, right, up
		...
		for(ii=0;ii<peaknum;ii++)
		{
			sprintf(paraprint,"Constant%d=%.1f%s%.3f",ii,par[ii][0]/binwidth,"+/-",parerr[0]/binwidth);//par数组还保持着刚才的参数
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Mean%d=%.1f%s%.3f",ii,par[ii][1],"+/-",parerr[1]);
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Sigma%d=%.1f%s%.3f",ii,par[ii][2],"+/-",parerr[2]);
			textgaus->AddText(paraprint);
		...
		sprintf(h_name,"D:/X/out/cali23Al/%s_%.1f.png",hfit_name,peakx[0]);//modify if change path
		canvaspeak[i]->SaveAs(h_name);//存图
		for(ii=0;ii<peaknum;ii++)
		{
			outfile<<hfit_name<<"	Constant"<<ii<<"=	"<<par[ii][0]<<"	Mean"<<ii<<"=	"<<par[ii][1]<<"	Sigma"<<ii<<"=	"<<par[ii][2]<<endl;//<<"	Area"<<ii<<"="<<sqrt(3.14159*2)*par[ii][2]*par[ii][0]/binwidth<<endl;//输出文本查看
			// For graph pol fit, mean is more accurate than peakx
		}
