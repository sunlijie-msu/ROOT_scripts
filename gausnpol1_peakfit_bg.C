...
#include "TLegend.h"
using namespace std;
void gausnpol1_peakfit_bg()// peak draw-fit (gausn+pol1) used for fit one peak, used to fit bg for fun
{//for 25Si known bg
...
//		histo[i]= (TH1F*)fin->Get("gbCoincidences"); //Get Gamma ray spectrum gated
	...
	{//don't change i=0 to other values
		...
		for(ii=2; ii<=2; ii++)//which peak in one SeGA detector modify =0<=5
		{
			//if(ii==4)continue;
...
// 			if(ii==5){	gaplow=25; gaphigh=25;}//23Al 7803
 			if(ii==0){	gaplow=8; gaphigh=8;}//25Si 452 old bg modify
...
// 			if(ii==9){	gaplow=44; gaphigh=44;}//25Si 7900-511*2
			...
			total[ii]=new TF1("total","gausn(0)+pol1(3)",peakx[ii]-gaplow,peakx[ii]+gaphigh);//高斯拟合，调节合适的拟合range
...
// 			b[ii]->SetNpx((gaphigh+gaplow)*10);
			...
			total[ii]->SetParameters(5000,945,1,800,-0.1);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
...
// 			total[ii]->SetParLimits(5,430,3000);//Mean  //Glassman
// 			total[ii]->SetParNames("BkgA","BkgB","Const*bin","Tau","Sigma","Mean");
			total[ii]->SetParNames("Const","Mean","Sigma","A","B");
...
// 			p[ii]->Draw("same");
// 			b[ii]->Draw("same");
			g[ii]->SetParameters(par[ii][0],par[ii][1],par[ii][2]);//set parameters for drawing
			g[ii]->SetLineColor(4);
			p[ii]->SetParameters(par[ii][3],par[ii][4]);//set parameters for drawing
			p[ii]->SetLineColor(8);
			g[ii]->Draw("same");
			p[ii]->Draw("same");
...
// 			textgaus->AddText(paraprint);
			...
			textgaus->AddText(paraprint);
			sprintf(paraprint,"p-val%d=%e",ii,p_value[ii]);
			textgaus->AddText(paraprint);
// 			sprintf(paraprint,"Maximum%d=%.4f%s%.4f",ii,peakx[ii],"+/-",peakxerr[ii]);//par数组还保持着刚才的参数
// 			textgaus->AddText(paraprint);
			sprintf(paraprint,"Constant%d=%.2f%s%.3f",ii,par[ii][0]/binwidth,"+/-",parerr[ii][0]/binwidth);//par数组还保持着刚才的参数
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Mean%d=%.2f%s%.3f",ii,par[ii][1],"+/-",parerr[ii][1]);
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Sigma%d=%.2f%s%.3f",ii,par[ii][2],"+/-",parerr[ii][2]);
			textgaus->AddText(paraprint);
			sprintf(paraprint,"Res%d=%.3f%%",ii,par[ii][2]/par[ii][1]*2.355*100);
			textgaus->AddText(paraprint);
			textgaus->Draw();
