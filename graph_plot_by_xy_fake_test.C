...
#include "TLegend.h"
using namespace std;
void graph_plot_by_xy_fake_test() // read in x y values from a txt file and plot a graph to view PXCT 237Np decay time fit results for fake test data generated by a random function
{
	const int row_start = 0;
	const int row_end = 80;
	const int rows_one_group = 40; // 40 or 28, one Ea gate has 40 rows of fitrange_min values
	...
	sprintf(filename, "%s%s", pathname, "x_T_y_fit_start_point_x_y_values_fake_test.txt");
	...
	//for (ii = 0; ii < rows_one_group; ii++)
	//{
	//	Ea_gate[ii] = Ea_gate_all[ii * 40 + iFitStart];
	//	fitrange_min[ii] = fitrange_min_all[ii * 40 + iFitStart];
	//	Half_life[ii] = Half_life_all[ii * 40 + iFitStart];
	//	Half_life_err[ii] = Half_life_err_all[ii * 40 + iFitStart];
	//	cout << "Ea_gate[" << ii << "]=" << Ea_gate[ii] << "	fitrange_min[" << ii << "]=" << fitrange_min[ii] << "	Half_life[" << ii << "]=" << Half_life[ii] << "	Half_life_err[" << ii << "]=" << Half_life_err[ii] << endl;
	//}
	...
	sprintf(graph_name, "%s%.0f%s", "Fake_test_", Ea_gate[0], "_");
	...
	graph->GetXaxis()->SetRangeUser(-10, 790);
	//graph->GetXaxis()->SetRangeUser(4, 62);
