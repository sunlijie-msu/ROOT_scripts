...
#include "TLegend.h"
using namespace std;
void graphexpfit_eff()// graph-fepol1 fit sigma, sqrt+const fit tau (with colored bands) of SeGA 16 channels obtained from Si25gausnerfcpol1_peakfit_response.C for 12 25Si,23Al peaks 
...
// 	}
	...
	{
		peaknum=nummax-1;//modify -1 for fitting, -0 for figure
// 		if(i==8||i==11||i==14|i==15) continue;
// 		if(i==5)peaknum=nummax-1;//modify
// 		if(i==0)peaknum=nummax-4;
		...
		cov.Print();
		cor.Print();
		r_sig->GetConfidenceIntervals(Npoint, 1, 1, energy_point, err_point, 0.683, false);//(Number of x points, 1, 1, x, err, confidence level, false); norm is a flag to control if the intervals need to be normalized to the chi2/ndf value. modify
		...
		p0[i]=fepol1->GetParameter(0);
		p1[i]=fepol1->GetParameter(1);
		p0err[i]=fepol1->GetParError(0);
