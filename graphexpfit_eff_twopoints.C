...
#include "TLegend.h"
using namespace std;
void graphexpfit_eff_twopoints()// graph-fepol1 fit sigma, sqrt+const fit tau (with colored bands) of SeGA 16 channels obtained from Si25gausnerfcpol1_peakfit_response.C for 12 25Si,23Al peaks 
{//also get the sigma and tau at four proton-gamma energies (1369,2754,2870,4238) for the simulation and uncertainty analysis.
	time_t tim;
	struct tm *at;
	char now[80];
	int irawroot;
	...
	const int nummax=3;//search peaknumber can be many, fitnumber can be limited to 3//no need to modify
	int peaknum=nummax;
...
// 	double energyliterr[6]={0.15, 0.005, 2, 0.010, 3, 2};//23Al
	...
	const int Npoint=18;
	double energy_point[Npoint] = {451.7, 493.3, 944.9, 1612.4, 1368.626, 2754.007, 2869.5, 4237.96, 844.6, 1337.8, 1789.4, 883.8, 6955, 2221.5, 2673.1, 6288, 7900, 7070};// all NuDat values, set location of point for single value modify
...
// 	energy[0][0]=451.547; energy[0][1]=492.982; energy[0][2]=1368.766876; //from Moshe
// 	energyerr[0][0]=0.532657; energyerr[0][1]=0.510226; energyerr[0][2]=0.52589; //from Moshe
// 	eff[0][0]=0.04178571; eff[0][1]=0.0375; eff[0][2]=0.027; //from Moshe
// 	efferr[0][0]=0.003535714; efferr[0][1]=0.003214286; efferr[0][2]=0.001; //from Moshe

	energy[0][0]=451.547; energy[0][1]=1368.766876; //from Moshe
	energyerr[0][0]=0.532657; energyerr[0][1]=0.52589; //from Moshe
	eff[0][0]=0.04178571; eff[0][1]=0.027; //from Moshe
	efferr[0][0]=0.003535714; efferr[0][1]=0.001; //from Moshe
...
// 		{
// 			infile>>energy[i][ii]>>energyerr[i][ii]>>eff[i][ii]>>efferr[i][ii]>>tau[i][ii]>>tauerr[i][ii];
// 			//cout<<"SeGA_"<<i<<'	'<<energy[i][ii]<<'	'<<energyerr[i][ii]<<'	'<<eff[i][ii]<<'	'<<efferr[i][ii]<<'	'<<tau[i][ii]<<'	'<<tauerr[i][ii]<<endl;
// 		}
...
// 	}
	...
	{
		peaknum=nummax-1;//modify nummax-1 for fitting, nummax-0 for figure
// 		if(i==8||i==11||i==14|i==15) continue;
// 		if(i==5)peaknum=nummax-1;//modify
// 		if(i==0)peaknum=nummax-4;
		...
		graph1[i]->GetYaxis()->SetTitle("Efficiency (frac.)");//轴名
		...
		graph1[i]->GetXaxis()->SetRangeUser(200,4500);
		graph1[i]->GetYaxis()->SetRangeUser(0.01,0.08);
		graph1[i]->SetMarkerStyle(21);
		graph1[i]->SetMarkerColor(1);
		TF1 *fepol1 = new TF1("fepol1","(exp([0]+[1]*log(x)+[2]*pow(log(x),2)+[3]*pow(log(x),3)+[4]*pow(log(x),4)+[5]*pow(log(x),5)+[6]*pow(log(x),6)))*[7]",10,10000);
		fepol1->SetParNames("p0","p1","p2","p3","p4","p5","p6","C");//y=exp(p0+p1*lnx+p2*(lnx)^2+...
		fepol1->SetParameter(0,1.777853);
		fepol1->SetParError(0,0.005598814);
		fepol1->SetParLimits(0,1.777853,1.777853);
		fepol1->SetParameter(1,-1.175923);
		fepol1->SetParError(1,0.0009436133);
		fepol1->SetParLimits(1,-1.175923,-1.175923);
		fepol1->SetParameter(2,1.106058);
		fepol1->SetParError(2,0.0001292948);
		fepol1->SetParLimits(2,1.106058,1.106058);
		fepol1->SetParameter(3,-0.6004948);
		fepol1->SetParError(3,1.688012e-05);
		fepol1->SetParLimits(3,-0.6004948,-0.6004948);
		fepol1->SetParameter(4,0.1250527);
		fepol1->SetParError(4,2.128892e-06);
		fepol1->SetParLimits(4,0.1250527,0.1250527);
		fepol1->SetParameter(5,-0.0112302);
		fepol1->SetParError(5,2.523441e-07);
		fepol1->SetParLimits(5,-0.0112302,-0.0112302);
		fepol1->SetParameter(6,0.0003661849);
		fepol1->SetParError(6,2.741992e-08);
		fepol1->SetParLimits(6,0.0003661849,0.0003661849);
		//graph1[i]->Fit("fepol1");//fepol1 can be used directly without TF1 constructor in CINT

		TFitResultPtr r_sig = graph1[i]->Fit("fepol1","MS");//"S" means the result of the fit is returned in the TFitResultPtr
		...
		{
			outfile<<"Eg=	"<<energy_point[ii]<<"	eff=	"<<fepol1->Eval(energy_point[ii])<<"	err_eff=	"<<err_point[ii]<<endl;
		}
		...
		p0[i]=fepol1->GetParameter(7);
//		p1[i]=fepol1->GetParameter(1);
		p0err[i]=fepol1->GetParError(7);
//		p1err[i]=fepol1->GetParError(1);
		parchi[i]=fepol1->GetChisquare();
		parNDF[i]=fepol1->GetNDF();
		p_value[i]=fepol1->GetProb();//This probability is not the “probability that your fit is good.” If you did many fake experiments (draw many random samples of data points from the assumed distribution (your fit function)), this is the percentage of experiments that would give χ2 values ≥ to the one you got in this experiment.
...
// 			//y[ii]=fepol1->Eval(energy_point[ii]);//is equal to y[ii]=p1[i]*x[ii]+p0[i];
// 			//dy[ii]=sqrt(x[ii]*x[ii]*p1err[i]*p1err[i]+p0err[i]*p0err[i]+p1[i]*p1[i]*dx[ii]*dx[ii]);
// 			//dy[ii]=err_point[ii];
// 			//cout<<"	"<<energy_point[ii]<<"	"<<err_point[ii]<<endl;
// 			errylow[ii]=fepol1->Eval(energy_point[ii])-err_point[ii];
// 			erryhigh[ii]=fepol1->Eval(energy_point[ii])+err_point[ii];
// 		}
...
// 		graph1bandhigh[i]->Draw("C");//"C" A smooth Curve is drawn
		...
		TPaveText *textpol1 = new TPaveText(0.10,0.64,0.34,0.89,"brNDC");//left, down, right, up
		...
		sprintf(paraprint,"y=C*x");
		textpol1->AddText(paraprint);
// 		sprintf(paraprint,"p1=%.9f+/-%.9f",p1[i],p1err[i]);
// 		textpol1->AddText(paraprint);
		...
		outfile2<<"SeGA_"<<i<<"	y=p1*x+p0	sigp1=	"<<setprecision(8)<<p1[i]<<"	+/-	"<<p1err[i]<<"	sigp0=	"<<p0[i]<<"	+/-	"<<p0err[i]<<"	Chi2=	"<<parchi[i]<<"	ndf=	"<<parNDF[i]<<"	p-value=	"<<p_value[i];
...
/*
		...
		graph3[i]= new TGraphErrors(peaknum,energy[i],tau[i],energyerr[i],tauerr[i]);//画error bars TGraph(n,x,y,ex,ey);
...
//		graph3[i]->Fit("func_sqrt_const");//fepol1 can be used directly without TF1 constructor in CINT
		...
		//canvascali[i]->SetGrid();//显示网格
		//		graph1[i]=new TGraph(peaknum,energy,tau);//TGraph *gr1=new TGraph(n,x,y);
		graph2[i]= new TGraphErrors(peaknum,energy[i],tau[i],energyerr[i],tauerr[i]);//画error bars TGraph(n,x,y,ex,ey);
...
//		graph2[i]->Fit("pol2");//fepol1 can be used directly without TF1 constructor in CINT
		...
		//canvascali[i]->SetGrid();//显示网格
		//		graph1[i]=new TGraph(peaknum,energy,tau);//TGraph *gr1=new TGraph(n,x,y);
		graph4[i]= new TGraphErrors(peaknum,energy[i],tau[i],energyerr[i],tauerr[i]);//画error bars TGraph(n,x,y,ex,ey);
...
//		graph4[i]->Fit("func_sqrt");//fepol1 can be used directly without TF1 constructor in CINT
