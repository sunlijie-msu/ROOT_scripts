...
#include "TLegend.h"
using namespace std;
void graphpol0scale_eff_four23Alpoints()// graph-fepol1 fit scale the SeGA efficiency curve (with colored bands) to match the four 23Al peaks 
{//also get the residuals at four gamma energies (451,1599,2904,7801) to verify the Gean4 simulation
	time_t tim;
	struct tm *at;
	char now[80];
	int irawroot;
...
// 	double energyliterr[6]={0.15, 0.005, 2, 0.010, 3, 2};//23Al
	...
	const int Npoint=4;
	double energy_point[Npoint] = {451,1599,2904,7801};// all NuDat values, set location of point for single value modify
...
// 	efferr[0][0]=0.003535714; efferr[0][1]=0.003214286; efferr[0][2]=0.001; //from Moshe
	...
	energy[0][0]=451; energy[0][1]=1599; energy[0][2]=2904; energy[0][3]=7801;
	energyerr[0][0]=0.15; energyerr[0][1]=2; energyerr[0][2]=2; energyerr[0][3]=2;
	eff[0][0]=0.0719; eff[0][1]=0.0362; eff[0][2]=0.0273; eff[0][3]=0.0074;
	efferr[0][0]=0.0027; efferr[0][1]=0.0015; efferr[0][2]=0.0026; efferr[0][3]=0.0006;
...
// 		if(i==0)peaknum=nummax-4;
		...
		graph1[i]->GetXaxis()->SetRangeUser(100,8000);
		graph1[i]->GetYaxis()->SetRangeUser(0.00,0.09);
		...
		{
			residual[i][ii]=(fepol1->Eval(energy_point[ii])-eff[i][ii])/fepol1->Eval(energy_point[ii]);
			residualerr[i][ii]=efferr[i][ii]/eff[i][ii];
...
// 		textpol1->AddText(paraprint);
		...
		//graph1[i]->GetYaxis()->SetLabelSize(0.05);//坐标字号
		//graph1[i]->GetYaxis()->SetTitleSize(0.05);//轴名字号
		graph2[i]->GetXaxis()->SetRangeUser(100,8000);
		graph2[i]->GetYaxis()->SetRangeUser(-0.5,0.5);
		...
		TLine *T1=new TLine(100,0,8000,0);
		T1->Draw("R");
/*
