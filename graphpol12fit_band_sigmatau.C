...
#include "TLegend.h"
using namespace std;
void graphpol12fit_band_sigmatau()// graph-pol1 fit sigma, sqrt+const fit tau (with colored bands) of SeGA 16 channels obtained from Si25gausnerfcpol1_peakfit_response.C for 12 25Si,23Al peaks 
{//also get the sigma and tau at four proton-gamma energies (1369,2754,2870,4238) for the simulation and uncertainty analysis.
	...
	const int nummax=8;//search peaknumber can be many, fitnumber can be limited to 3//no need to modify
	int peaknum=nummax;
...
//		sprintf(b_name,"%s%d%s","D:/X/out/Si25/SeGA_",i,"_sigma_freetau.dat");//modify 12 peaks
	...
	for(i=13;i<=13;i++)//which SeGA detector modify =0<=15
	{
		peaknum=nummax;
		if(i==8||i==11||i==14||i==15) continue;
		if(i==5)peaknum=nummax-1;//modify
		...
		sprintf(hcali_name,"%s%d","Sigma of SeGA",i);
		...
		TMatrixD cor = r_sig->GetCorrelationMatrix();
		TMatrixD cov = r_sig->GetCovarianceMatrix();
		cov.Print();
		cor.Print();
		...
		{
			outfile<<"SeGA_"<<i<<"	Eg=	"<<energy_point[ii]<<"	sig=	"<<pol1->Eval(energy_point[ii])<<"	err_sig=	"<<err_point[ii]<<endl;
		}
