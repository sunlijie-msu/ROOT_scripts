...
#include "TLegend.h"
using namespace std;
void graphpol12fit_band_sigmatau_RIBLL()// graph-pol1 fit sigma, sqrt+const fit tau (with colored bands) of Clover 5 channels obtained from Si25gausnerfcpol1_peakfit_response_RIBLL.C for 4 25Si peaks 
{
	time_t tim;
	struct tm *at;
	char now[80];
	...
	double gaplow=70.,gaphigh=70.;//fitting range随分辨不同调整//no need to modify
	unsigned long i;
	...
	double mean[ID2+1][nummax],meanerr[ID2+1][nummax];
...
//		sprintf(b_name,"%s%d%s","D:/X/out/Si25/SeGA_",i,"_sigma_freetau.dat");//modify 12 peaks
		sprintf(b_name,"%s%d%s","D:/X/out/Si25/Clover_",i,"_sigma_tau_onlybg_RIBLL.dat");//only 4 beta-delayed gamma peaks
		...
		for(ii=0;ii<peaknum;ii++)//which peak in one SeGA detector modify =0<12
		{
			...
			//cout<<"Clover_"<<i<<'	'<<mean[i][ii]<<'	'<<meanerr[i][ii]<<'	'<<sig[i][ii]<<'	'<<sigerr[i][ii]<<'	'<<tau[i][ii]<<'	'<<tauerr[i][ii]<<endl;
		}
		if(i==0) {range_sig1[i]=0.7; range_sig2[i]=3.5; range_tau1[i]=-0.5; range_tau2[i]=5;}
		if(i==1) {range_sig1[i]=0.7; range_sig2[i]=3.5; range_tau1[i]=-0.5; range_tau2[i]=5;}
		if(i==2) {range_sig1[i]=0.7; range_sig2[i]=3.5; range_tau1[i]=-0.5; range_tau2[i]=5;}
		if(i==3) {range_sig1[i]=0.7; range_sig2[i]=3.5; range_tau1[i]=-0.5; range_tau2[i]=5;}
		if(i==4) {range_sig1[i]=0.7; range_sig2[i]=3.5; range_tau1[i]=-0.5; range_tau2[i]=5;}
	}
...
// 		if(i==0)peaknum=nummax-4;
		...
		sprintf(hcali_name,"%s%d","Sigma of Clover",i);
		...
		graph1[i]->GetXaxis()->SetRangeUser(0,4500);
		...
		TFitResultPtr r_sig = graph1[i]->Fit("pol1","MS");//"S" means the result of the fit is returned in the TFitResultPtr
		...
		r_sig->GetConfidenceIntervals(Npoint, 1, 1, energy_point, err_point, 0.683, false);//(Number of x points, 1, 1, x, err, confidence level, false); norm is a flag to control if the intervals need to be normalized to the chi2/ndf value.
		for(ii=0;ii<Npoint;ii++)
		{
			outfile<<"Clover_"<<i<<"	Eg=	"<<energy_point[ii]<<"	sig=	"<<pol1->Eval(energy_point[ii])<<"	err_sig=	"<<err_point[ii]<<endl;
		}
...
// 		cout<<sqrt(abs(E_mu_mu))<<endl;
		...
		}//only four points at pg energies, not for drawing
...
// 		graph1bandhigh[i]->Draw("C");//"C" A smooth Curve is drawn
		...
		outfile2<<"Clover_"<<i<<"	y=p1*x+p0	sigp1=	"<<setprecision(8)<<p1[i]<<"	+/-	"<<p1err[i]<<"	sigp0=	"<<p0[i]<<"	+/-	"<<p0err[i]<<"	Chi2=	"<<parchi[i]<<"	ndf=	"<<parNDF[i]<<"	p-value=	"<<p_value[i];
		...
		sprintf(hcali_name,"%s%d","Tau of Clover",i);
		canvascali[i]=new TCanvas(hcali_name,hcali_name,900,600);//建立画布
		canvascali[i]->cd();//进入画布
		//canvascali[i]->SetGrid();//显示网格
		//		graph1[i]=new TGraph(peaknum,mean,tau);//TGraph *gr1=new TGraph(n,x,y);
		graph3[i]= new TGraphErrors(peaknum,mean[i],tau[i],meanerr[i],tauerr[i]);//画error bars TGraph(n,x,y,ex,ey);
...
//		graph3[i]->Fit("func_sqrt_const");//pol1 can be used directly without TF1 constructor in CINT
		...
		for(ii=0;ii<Npoint;ii++)
		{
			outfile<<"Clover_"<<i<<"	Eg=	"<<energy_point[ii]<<"	tau=	"<<func_sqrt_const->Eval(energy_point[ii])<<"	err_tau=	"<<err_point[ii]<<endl;
		}
...
// 		graph3bandhigh[i]->Draw("C");
		...
		sprintf(hcali_name,"%s%s%s%s%s","D:/X/out/responseforDoppler/",hcali_name,"_",tauflag,"_band.png");
		canvascali[i]->SaveAs(hcali_name);//存图
		outfile2<<"	Clover_"<<i<<"	y=p1*sqrt(x)+p0	taup1=	"<<setprecision(8)<<p1[i]<<"	+/-	"<<p1err[i]<<"	taup0=	"<<p0[i]<<"	+/-	"<<p0err[i]<<"	Chi2=	"<<parchi[i]<<"	ndf=	"<<parNDF[i]<<"	p-value=	"<<p_value[i]<<endl;
...
//		graph2[i]->Fit("pol2");//pol1 can be used directly without TF1 constructor in CINT
		...
		//canvascali[i]->SetGrid();//显示网格
		//		graph1[i]=new TGraph(peaknum,mean,tau);//TGraph *gr1=new TGraph(n,x,y);
		graph4[i]= new TGraphErrors(peaknum,mean[i],tau[i],meanerr[i],tauerr[i]);//画error bars TGraph(n,x,y,ex,ey);
