...
#include "TLegend.h"
using namespace std;
void graphpol1fit_band_energy_cali()// read in 12 .dat files obtained from Si25gausnerfcpol1_peakfit_cali.C and graph-pol1 fit maximum (with colored bands) of SeGA 16 channels for 12 25Si,23Al peaks to extract formal ax+b parameters from fitting.
{
	time_t tim;
	struct tm *at;
	char now[80];
	...
	const int nummax=4;//search peaknumber can be many, fitnumber can be limited to 3//modify
	int peaknum;
...
//	double energy_point[Npoint] = {451.7820, 493.1847, 944.6500, 1460.8200, 1612.6869, 2614.5110, 450.6924, 1599.5500, 2904.7794, 7801.4706, 1369, 2754, 2870, 4238};// set location of point for single value
	double energy_point[Npoint] = {845, 1338, 1789, 1460.8200, 884, 2614.5110, 10000, 10000, 10000, 7900, 1728, 2222, 2673, 6288};// set location of point for single value modify whose uncertainty do you want?
...
//	int lowerlimit[ID2+1][nummax],upperlimit[ID2+1][nummax];
	...
	for(i=0;i<=0;i++)//which SeGA detector modify =0<=15
	{
		...
		if(i==8) continue;
		//if(i==5)peaknum=nummax-1;
		sprintf(hcali_name,"%s%d","AlSi_Maximum_SeGA_",i);
		...
		TMatrixD cor = r_cali->GetCorrelationMatrix();//parameter correlation coefficients
		TMatrixD cov = r_cali->GetCovarianceMatrix();//error matrix
		cov.Print();
		cor.Print();
