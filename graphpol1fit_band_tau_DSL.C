...
#include "TLegend.h"
using namespace std;
void graphpol1fit_band_tau_DSL()// graph-pol1 fit sigma and tau (with colored bands) of GRIFFIN obtained from gausnerfcpol1_peakfit_response_DSL.C for 6 unshifited peaks
{//also get the sigma and tau at seven 31S gamma energies (1248,2234,3076,4971,5156,6392,6542) for the simulation and uncertainty analysis.
	time_t tim;
	struct tm *at;
	char now[80];
	int irawroot;
...
// 		if(i==0)peaknum=nummax-4;
		sprintf(b_name,"%s%d%s","D:/X/out/DSL/DSL_sigma_tau_addback",i,"deg.dat");
		ifstream infile(b_name,ios::in);//The data that need to be fitted
		for(ii=0;ii<peaknum;ii++)//which peak in one GRIFFIN detector modify =0<12
		{
			infile>>energy[i][ii]>>energyerr[i][ii]>>sig[i][ii]>>sigerr[i][ii]>>tau[i][ii]>>tauerr[i][ii];
			cout<<"GRIFFIN_"<<i<<'	'<<energy[i][ii]<<'	'<<energyerr[i][ii]<<'	'<<sig[i][ii]<<'	'<<sigerr[i][ii]<<'	'<<tau[i][ii]<<'	'<<tauerr[i][ii]<<endl;
		}
		if(i==0) {range_sig1[i]=0.5; range_sig2[i]=2.5; range_tau1[i]=-1; range_tau2[i]=6;}
...
// 		if(i==0)peaknum=nummax-4;
		...
		//		graph1[i]=new TGraph(peaknum,energy,sig);//TGraph *gr1=new TGraph(n,x,y);
		graph1[i]= new TGraphErrors(peaknum,energy[i],tau[i],energyerr[i],tauerr[i]);//画error bars TGraph(n,x,y,ex,ey);
		graph1[i]->SetTitle(hcali_name);
		graph1[i]->GetXaxis()->SetTitle("E_{#gamma} (keV)");//轴名
		graph1[i]->GetYaxis()->SetTitle("Tau");//轴名
...
//		graph1[i]->GetYaxis()->SetRangeUser(0.9,1.5);
		...
		TH1D *hint_tau = new TH1D("hint_tau", "Fitted exponential with conf.band", 9000, 0, 9000);//Create a histogram to hold the confidence intervals
		(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau, 0.683);//By default the intervals are corrected using the chi2/ndf value of the fit if a chi2 fit is performed
		//hint_tau will contain the CL result that you can draw on top of your fitted graph.
		//where hint_tau will hold the errors and could superimpose it on the same canvas where you plot central values.
		//The method TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau, 0.95) computes the confidence level of your model (fitting) function after having it fitted to an histogram.
		hint_tau->SetStats(kFALSE);
		hint_tau->SetFillColor(kRed-9);
		//TMatrixDSym cov = r_tau->GetCovarianceMatrix();//useless for now
		TMatrixD cov = r_tau->GetCorrelationMatrix();
		TMatrixD cor = r_tau->GetCovarianceMatrix();
		cov.Print();
		cor.Print();
		r_tau->GetConfidenceIntervals(Npoint, 1, 1, energy_point, err_point, 0.683, true);//(Number of x points, 1, 1, x, err, confidence level, false); norm is a flag to control if the intervals need to be normalized to the chi2/ndf value. modify
		for(ii=0;ii<Npoint;ii++)//get sigma/tau at energies of interest
		{
			outfile<<"GRIFFIN_"<<i<<"	Eg=	"<<energy_point[ii]<<"	tau=	"<<pol1->Eval(energy_point[ii])<<"	err_tau=	"<<err_point[ii]<<endl;
			//outfile<<"Eg=	"<<energy_point[ii]<<"	relative residual=	"<<residual[i][ii]<<endl;
		}
		for(ii=0;ii<peaknum;ii++)//get sigma/tau at energies for plotting the residuals
		{
			residual[i][ii]=(pol1->Eval(energy[i][ii])-tau[i][ii])/pol1->Eval(energy[i][ii]);
			residualerr[i][ii]=tauerr[i][ii]/pol1->Eval(energy[i][ii]);
		}
		graph1[i]->Draw("AP");//"A": Axis are drawn around the graph, "P": The current marker is plotted at each point
		hint_tau->Draw("e3 same");
...
// 			//y[ii]=pol1->Eval(energy_point[ii]);//is equal to y[ii]=p1[i]*x[ii]+p7[i];
// 			//dy[ii]=sqrt(x[ii]*x[ii]*p1err[i]*p1err[i]+p7err[i]*p7err[i]+p1[i]*p1[i]*dx[ii]*dx[ii]);
// 			//dy[ii]=err_point[ii];
// 			//cout<<"	"<<energy_point[ii]<<"	"<<err_point[ii]<<endl;
// 			errylow[ii]=pol1->Eval(energy_point[ii])-err_point[ii];
// 			erryhigh[ii]=pol1->Eval(energy_point[ii])+err_point[ii];
// 		}
...
// 		graph1bandhigh[i]->Draw("C");//"C" A smooth Curve is drawn
		...
		sprintf(hcali_name,"%s%s%s%s%s","D:/X/out/Figure_DSL/",hcali_name,"_",tauflag,"_tau_band.png");
		canvascali[i]->SaveAs(hcali_name);//存图
		outfile2<<"GRIFFIN_"<<i<<"	y=p1*x+p0	taup1=	"<<setprecision(8)<<p1[i]<<"	+/-	"<<p1err[i]<<"	taup0=	"<<p0[i]<<"	+/-	"<<p0err[i]<<"	Chi2=	"<<parchi[i]<<"	ndf=	"<<parNDF[i]<<"	p-value=	"<<p_value[i];
// 		outfile2<<"SeGA_"<<i<<"	y=pol1*x	"<<endl;
