...
#include "TLegend.h"
using namespace std;
void graphpol45fit_band_efficiency_curve()// graph-pol45 fit efficiency curve (with colored bands) of Proton Detector
{
	time_t tim;
	struct tm *at;
	char now[80];
...
// 	double energyliterr[6]={0.15, 0.005, 2, 0.010, 3, 2};//23Al
	...
	const int Npoint=10;//modify
	double energy_point[Npoint] = {401, 555, 943, 1040, 1268, 1377, 1804, 1917, 2162, 2307};// set location of point for single value
...
// 		}
 		if(i==0) {range_x1[i]=0; range_x2[i]=2500; range_y1[i]=0; range_y2[i]=0.50;}
...
// 		if(i==13) {range_x1[i]=-4; range_x2[i]=7; range_y1[i]=-2; range_y2[i]=6;}
		...
		Efficiency[0][0]=37.49/100.0; Efficiencyerr[0][0]=9.65/100.0;
		Efficiency[0][1]=34.20/100.0; Efficiencyerr[0][1]=4.65/100.0;
		Efficiency[0][2]=20.65/100.0; Efficiencyerr[0][2]=5.83/100.0;
		Efficiency[0][3]=12.89/100.0; Efficiencyerr[0][3]=5.35/100.0;
		Efficiency[0][4]=6.31/100.0; Efficiencyerr[0][4]=3.53/100.0;
		Efficiency[0][5]=5.71/100.0; Efficiencyerr[0][5]=2.51/100.0;
		Efficiency[0][6]=3.50/100.0; Efficiencyerr[0][6]=1.32/100.0;
		Efficiency[0][7]=2.17/100.0; Efficiencyerr[0][7]=0.62/100.0;
		Efficiency[0][8]=1.31/100.0; Efficiencyerr[0][8]=0.40/100.0;
		Efficiency[0][9]=0.87/100.0; Efficiencyerr[0][9]=0.38/100.0;
	}
...
// 		graph3bandhigh[i]->Draw("C");
		...
		outfile2<<"	pol4_"<<i<<"	y=p2*x^2+p1*x+p0	p2=	"<<setprecision(8)<<p2[i]<<"	+/-	"<<p2err[i]<<"	p1=	"<<p1[i]<<"	+/-	"<<p1err[i]<<"	p0=	"<<p0[i]<<"	+/-	"<<p0err[i]<<"	Chi2=	"<<parchi[i]<<"	ndf=	"<<parNDF[i]<<"	p-value=	"<<p_value[i]<<endl;
				...
				//************ Efficiency fit by pol5 *****************************************

		if(i==0)sprintf(hcali_name,"%s","pol5");
		canvascali[i]=new TCanvas(hcali_name,hcali_name,900,600);//建立画布
		canvascali[i]->cd();//进入画布
		//canvascali[i]->SetGrid();//显示网格
		//		graph1[i]=new TGraph(peaknum,Energy,Efficiency);//TGraph *gr1=new TGraph(n,x,y);
		...
		TF1 *pol5=new TF1("pol5","pol5",-60000,60000);//多项式拟合，调节合适的拟合range
		pol5->SetParNames("p0","p1","p2","p3","p4","p5");//y=p3*x^2+p2*x^2+p1*x+p0
		pol5->SetParameter(5,1.41E-16);
		pol5->SetParameter(4,-1.15E-12);
		pol5->SetParameter(3,3.50E-09);
		pol5->SetParameter(2,-4.72E-06);
		pol5->SetParameter(1,2.43E-03);
		pol5->SetParameter(0,-4.24E-02);

//		graph2[i]->Fit("pol4");//pol1 can be used directly without TF1 constructor in CINT

		TFitResultPtr r_tau_pol5 = graph3[i]->Fit("pol5","MS");//"S" means the result of the fit is returned in the TFitResultPtr
		//“M” Improve fit results, by using the IMPROVE algorithm of TMinuit.
		//If the option "S" is instead used, TFitResultPtr contains the TFitResult and behaves as a smart pointer to it.
		//The fit parameters, error and chi2 (but not covariance matrix) can be retrieved also from the fitted function.
		TH1D *hint_tau_pol5 = new TH1D("hint_tau_pol5", "Fitted exponential with conf.band", 9000, 0, 9000);//Create a histogram to hold the confidence intervals
		(TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_tau_pol5, 0.683);//By default the intervals are corrected using the chi2/ndf value of the fit if a chi2 fit is performed
		//hint_tau_pol4 will contain the CL result that you can draw on top of your fitted graph.
		//where hint_sig will hold the errors and could superimpose it on the same canvas where you plot central values.
		//The method TVirtualFitter::GetFitter())->GetConfidenceIntervals(hint_sig, 0.95) computes the confidence level of your model (fitting) function after having it fitted to an histogram.
		hint_tau_pol5->SetStats(kFALSE);
		hint_tau_pol5->SetFillColor(kRed-9);
		//TMatrixDSym cov = r_tau_pol4->GetCovarianceMatrix();//useless for now
		TMatrixD cov = r_tau_pol5->GetCorrelationMatrix();
		TMatrixD cor = r_tau_pol5->GetCovarianceMatrix();
		cov.Print();//get cov=rou*sigma1*sigma2...
		cor.Print();//get rou
		r_tau_pol5->GetConfidenceIntervals(Npoint, 1, 1, energy_point, err_point, 0.683, false);//(Number of x points, 1, 1, x, err, confidence level, norm); norm is a flag to control if the intervals need to be normalized to the chi2/ndf value.
		for(ii=0;ii<Npoint;ii++)
		{
			outfile<<"pol5_"<<i<<"	Energy=	"<<energy_point[ii]<<"	Efficiency=	"<<pol5->Eval(energy_point[ii])<<"	err_Y=	"<<err_point[ii]<<endl;
		}
		graph3[i]->Draw("AP");//"A": Axis are drawn around the graph, "P": The current marker is plotted at each point
		hint_tau_pol5->Draw("e3 same");
		graph3[i]->Draw("P same");//draw the points again above error band

		p0[i]=pol5->GetParameter(0);
		p0err[i]=pol5->GetParError(0);
		p1[i]=pol5->GetParameter(1);
		p1err[i]=pol5->GetParError(1);
		p2[i]=pol5->GetParameter(2);
		p2err[i]=pol5->GetParError(2);
		p3[i]=pol5->GetParameter(3);
		p3err[i]=pol5->GetParError(3);
		p4[i]=pol5->GetParameter(4);
		p4err[i]=pol5->GetParError(4);
		p5[i]=pol5->GetParameter(5);
		p5err[i]=pol5->GetParError(5);
		parchi[i]=pol5->GetChisquare();
		parNDF[i]=pol5->GetNDF();
		p_value[i]=pol5->GetProb();//This probability is not the “probability that your fit is good.” If you did many fake experiments (draw many random samples of data points from the assumed distribution (your fit function)), this is the percentage of experiments that would give χ2 values ≥ to the one you got in this experiment.
		double errylow[Npoint],erryhigh[Npoint];
// 		for(ii=0;ii<Npoint;ii++)
// 		{
// 			//x[ii]=energy_point[ii];
...
// 		graph3bandhigh[i]->Draw("C");
		...
		TPaveText *textpol5 = new TPaveText(0.54,0.58,0.90,0.89,"brNDC");//left, down, right, up
		textpol5->SetBorderSize(1);
		textpol5->SetFillColor(0);
		textpol5->SetTextAlign(12);//align = 10*HorizontalAlign + VerticalAlign, 12 means水平左对齐、垂直居中对齐
		textpol5->SetTextFont(132);//font = 10 * fontID + precision, 12+2,12 means Symbol; 13+2, 13 means News Times Roman
		sprintf(paraprint,"y=p5*x^5+p4*x^4+p3*x^3+p2*x^2+p1*x+p0");
		textpol5->AddText(paraprint);
		...
		sprintf(hcali_name,"%s%s%s%s%s","D:/X/out/",hcali_name,"_",tauflag,"_band.png");
		canvascali[i]->SaveAs(hcali_name);//存图
		outfile2<<"	pol5_"<<i<<"	y=p3*x^3+p2*x^2+p1*x+p0	p3=	"<<setprecision(8)<<p3[i]<<"	+/-	"<<p3err[i]<<"	p2=	"<<p2[i]<<"	+/-	"<<p2err[i]<<"	p1=	"<<p1[i]<<"	+/-	"<<p1err[i]<<"	p0=	"<<p0[i]<<"	+/-	"<<p0err[i]<<"	Chi2=	"<<parchi[i]<<"	ndf=	"<<parNDF[i]<<"	p-value=	"<<p_value[i]<<endl;
