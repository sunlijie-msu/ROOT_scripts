...
#include "TLegend.h"
using namespace std;
void graphpol6fit_Chi2_DSL_varyTau()// graph-pol6 fit Chi2 in D:\X\out\DSL\Chi2, the file name is a bit messy
{
	time_t tim;
	struct tm *at;
	char now[80];
...
// 	for(jj=0;jj<Npoint;jj++){	energy_point[jj]=jj*40;}
	...
	int Eg=0; //DSLmodify always finite value
	...
	if (ipeak==0) { Eg=1248; Ea=49; Egv=1248; minrange=500; maxrange=1600; }
	if (ipeak==1) { Eg=2234; Ea=47; Egv=2234; minrange=50; maxrange=550; }
	if (ipeak==2) { Eg=3076; Ea=46; Egv=3076; minrange=0; maxrange=30; }
	if (ipeak==3) { Eg=4971; Ea=42; Egv=4970; minrange=0; maxrange=20; }
	if (ipeak==4) { Eg=5156; Ea=42; Egv=5156; minrange=0; maxrange=30; }
	if (ipeak==5) { Eg=5141; Ea=39; Egv=5141; minrange=0; maxrange=30; }
	if (ipeak==6) { Eg=4156; Ea=39; Egv=4156; minrange=0; maxrange=20; }
...
//	int lowerlimit[ID2+1][nummax],upperlimit[ID2+1][nummax];
	...
	for(int irun=7;irun<=7;irun++)//do Chi minimization fit for each variable
	{// DSLmodify choose a flag
		...
		if (irun==5) sprintf(flag,"Eglow");
		if (irun==6) sprintf(flag,"Eghigh");
		if (irun==7) sprintf(flag,"Eglow0.2keV");
		if (irun==8) sprintf(flag,"Eghigh0.2keV");
		if (irun==9) sprintf(flag,"AC2-0.8");
// 		if (irun==8) sprintf(flag,"respsigma1");
// 		if (irun==9) sprintf(flag,"respsigma-1");
// 		if (irun==10) sprintf(flag,"resptau1");
// 		if (irun==11) sprintf(flag,"resptau-1");
//		if (irun==12) sprintf(flag,"G4");
...
//		sprintf(b_name,"%s%d%s%d%s","D:/X/out/DSL/Chi2_Ea",Ea,"Eg",Eg,"projection_vTau.dat");//DSLmodify
		sprintf(b_name,"%s%d%s%d%s%.0f%s%s%s","D:/X/out/DSL/Chi2/Chi2_Gamma",Eg,"_Ea",Ea,"_Egv",Egv,"slice_vTau_",flag,".dat");
		cout<<b_name<<endl;
		...
		{
			if (irow++ <0) continue; // <3 means to skip the first 3 rows
			stringstream(line)>>Energy[i][ii]>>Chi[i][ii];
			cout<<Energy[i][ii]<<'	'<<Chi[i][ii]<<endl;
			ii++;
		}
		peaknum=ii-1;// sometimes the last line is empty and should be discarded, or it will add a (0,0) point to the graph
	}
	// 	const int Npoint=4;
	// 	double energy_point[Npoint] = {1369,2754,2870,4238};// set location of point for single value
	// 	double err_point[Npoint];  // error on the function at point x0 for single value
...
// 		if(i==0)peaknum=nummax-4;
		...
		sprintf(hcali_name,"%s%d%s%d%s%.0f%s%s","Chi2_Gamma",Eg,"_Ea",Ea,"_Egv",Egv,"slice_vTau_",flag);
		...
		graph1[i]->GetYaxis()->SetTitle("#chi^{2}");//轴名
		...
		graph1[i]->GetXaxis()->SetLabelSize(0.05);//坐标字号
		graph1[i]->GetXaxis()->SetTitleSize(0.06);//轴名字号
		graph1[i]->GetYaxis()->SetLabelSize(0.05);//坐标字号
		graph1[i]->GetYaxis()->SetTitleSize(0.06);//轴名字号
		graph1[i]->GetXaxis()->SetTitleOffset(1.0);//轴名偏移
		graph1[i]->GetYaxis()->SetTitleOffset(0.9);//轴名偏移
...
// 		if(Eg==2814){ minrange=2812; maxrange=2816; }
		...
		//DSLmodify
// 		pol6->SetParameter(6,8.06155953130085E-16);
// 		pol6->SetParameter(5,-6.93086833688022E-12);
// 		pol6->SetParameter(4,2.47537018918513E-08);
// 		pol6->SetParameter(3,-4.70589311223611E-05);
// 		pol6->SetParameter(2,5.03013267062659E-02);
// 		pol6->SetParameter(1,-2.80301212664946E+01);
// 		pol6->SetParameter(0,1.26935344413607E+04);
...
// 		pol6->SetParameter(6,9.68031e-014);
// 		pol6->SetParameter(5,-1.52618e-011);
// 		pol6->SetParameter(4,-2.12054e-006);
// 		pol6->SetParameter(3,-0.0174038);
// 		pol6->SetParameter(2,-79.6153);
// 		pol6->SetParameter(1,-22982.9);
// 		pol6->SetParameter(0,5.13151e+009);

// 		//y = 2.4161E-01x6 - 8.8833E+03x5 + 1.3609E+08x4 - 1.1119E+12x3 + 5.1104E+15x2 - 1.2526E+19x + 1.2794E+22
// 		pol6->SetParLimits(6,2.4161E-01,2.4161E-01);
// 		pol6->SetParLimits(5,-8.8833E+03,-8.8833E+03);
// 		pol6->SetParLimits(4,1.3609E+08,1.3609E+08);
// 		pol6->SetParLimits(3,-1.1119E+12,-1.1119E+12);
// 		pol6->SetParLimits(2,5.1104E+15,5.1104E+15);
// 		pol6->SetParLimits(1,-1.2526E+19,-1.2526E+19);
// 		pol6->SetParLimits(0,1.2794E+22,1.2794E+22);
...
// 		textpol6->AddText(paraprint);
		...
		outfile2<<x<<"	"<<x-xlow<<"	"<<xhigh-x<<endl;
