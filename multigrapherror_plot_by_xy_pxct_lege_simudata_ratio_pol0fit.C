...
void multigrapherror_plot_by_xy_pxct_lege_simudata_ratio_pol0fit() // reading x and y values from a file where the first column is x values and the subsequent columns are y values
// https://root.cern.ch/doc/master/classTMultiGraph.html
{
	char pathname[300], filename[300], detectorname[300], figurename[300];
	vector<string> detector_name = { "LEGe","XtRa2" };
...
// 		}
		...
		const int Npoint = 4;
		double energy_point[Npoint] = { 6, 7, 8, 9 };// set location of point for single value. modify
	...
	multigraph->GetXaxis()->SetLimits(0, 10);
	multigraph->GetXaxis()->SetRangeUser(0, 10);
	...
	graph_residual[0]->GetXaxis()->SetLimits(0, 10);
	graph_residual[0]->GetXaxis()->SetRangeUser(0, 10);
	...
	graph_residual[1]->GetXaxis()->SetLimits(0, 10);
	graph_residual[1]->GetXaxis()->SetRangeUser(0, 10);
...
// 	colorText->Draw();
	...
	canvaspeak->cd();
	canvaspeak->Update();
	sprintf(filename, "%s%s", pathname, "Fig_PXCT_Gamma_Efficiency_SimuData_Ratio_LEGe");
