...
#include "TLegend.h"
using namespace std;
void peakcali_pol1_band_e21010()//for linear calibration with a credible interval band
{
	...
	const int num_datapoints = 14;//search peak numbers, modify
	...
	for (i = ID1; i <= ID2; i++)
	{
		sprintf(filename, "%s%s%d%s", pathname, "y_values_G", i, ".dat");
		ifstream infile_y_values(filename, ios::in);
		sprintf(filename, "%s%s%d%s", pathname, "x_values_G", i, ".dat");
		...
		for (ii = 0; ii < num_datapoints; ii++)
		{
			...
			cout << "G" << i << '	' << ii << '	' << x_value[i][ii] << '	' << x_error[i][ii] << '	' << y_value[i][ii] << '	' << y_error[i][ii] << endl;
		}
	}
	...
	{
		sprintf(hcali_name, "%s%d", "Energy_calibration_G", i);
		...
		graph[i]->GetYaxis()->SetTitle("Literature energy (keV)");
		...
		graph[i]->GetXaxis()->SetRangeUser(0, 3700);
		graph[i]->SetMarkerStyle(21);
		graph[i]->SetMarkerColor(1);

		TF1* pol1 = new TF1("pol1", "pol1", 0, 4000);
		pol1->SetNpx(40000);
		...
		h_confidence_interval[i] = new TH1D(filename, filename, 40000, 0, 4000);//Create a histogram to hold the confidence intervals
		...
		TLine* T1 = new TLine(0, 0, 3600, 0); // TLine(x1, y1, x2, y2)
		T1->Draw("R");

		sprintf(filename, "%s%s%s", pathname, hcali_name, ".png");
		canvascali[i]->SaveAs(filename);
		outfile << "G" << i << "	y=p1*x+p0	p1=	" << setprecision(8) << par[i][1] << "	+/-	" << par_err[i][1] << "	p0=	" << par[i][0] << "	+/-	" << par_err[i][0] << "	Chi2=	" << parChi[i] << "	ndf=	" << parNDF[i] << "	p-value=	" << p_value[i] << endl;
