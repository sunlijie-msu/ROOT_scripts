...
#include "TLegend.h"
using namespace std;
void peakcali_pol1_band_pxct_MSD26()//for linear calibration with a credible interval band
{
	...
	double par[ID2 + 1][2], par_err[ID2 + 1][2];
	double parChi[ID2 + 1], parNDF[ID2 + 1], p_value[ID2 + 1];

	const int Npoint = 3;
	double energy_point[Npoint] = { 31300, 31350, 31400 };// set location of point for single value. modify
	...
	for (i = ID1; i <= ID2; i++)
	{
		sprintf(filename, "%s%s%d%s", pathname, "y_values_MSD26_", i, ".dat");
		ifstream infile_y_values(filename, ios::in);
		sprintf(filename, "%s%s%d%s", pathname, "x_values_MSD26_", i, ".dat");
		...
		for (ii = 0; ii < num_datapoints; ii++)
		{
			...
			cout << "MSD26_" << i << '	' << ii << '	' << x_value[i][ii] << '	' << x_error[i][ii] << '	' << y_value[i][ii] << '	' << y_error[i][ii] << endl;
		}
	}
	...
	{
		sprintf(hcali_name, "%s%d", "Energy_calibration_MSD26", i);
		...
		graph[i]->GetXaxis()->SetTitle("ADC channel");
		graph[i]->GetYaxis()->SetTitle("#alpha energy (keV)");
		...
		graph[i]->GetXaxis()->SetRangeUser(25000, 50000);
		graph[i]->SetMarkerStyle(21);
		graph[i]->SetMarkerColor(1);

		TF1* pol1 = new TF1("pol1", "pol1", 0, 50000);
		pol1->SetNpx(50000);
		pol1->SetParNames("p0", "p1");//y=p0+p1*x
		//graph[i]->Fit("pol1");//pol1 can be used directly without TF1 constructor in CINT
		TFitResultPtr Fit_result_pointer = graph[i]->Fit("pol1", "MS");
		...
		h_confidence_interval[i] = new TH1D(filename, filename, 50000, 0, 50000);//Create a histogram to hold the confidence intervals
		...
		pol1->GetParameters(par[i]);
		par_err[i][0] = pol1->GetParError(0);
		par_err[i][1] = pol1->GetParError(1);
		...
		Fit_result_pointer->GetConfidenceIntervals(Npoint, 1, 1, energy_point, err_point, 0.95, true);//get the error of FWHM at energies of interest, this command is independent of the colored band
		//(Number of x points, 1, 1, x, err, confidence level, false); norm is a flag to control if the intervals need to be inflated by the chi2/ndf value. true is inflated, false is not inflated.
		...
		{
			cout << "X_value=	" << energy_point[ipeak] << "	Y_value=	" << pol1->Eval(energy_point[ipeak]) << "	err_Y_value=	" << err_point[ipeak] << endl;
		}
		...
		{
			residual[i][ii] = (pol1->Eval(x_value[i][ii]) - y_value[i][ii]);
		...
		graph_residual[i]->GetXaxis()->SetTitle("ADC channel");
		...
		graph_residual[i]->GetXaxis()->SetRangeUser(25000, 50000);
...
// 		graph_residual[i]->GetXaxis()->SetNdivisions(10, 10, 1);
		...
		TLine* T1 = new TLine(25137, 0, 49689, 0); // TLine(x1, y1, x2, y2)
		T1->Draw("R");

		sprintf(filename, "%s%s%s", pathname, hcali_name, ".png");
		canvascali[i]->SaveAs(filename);
		outfile << "MSD26_" << i << "	y=p1*x+p0	p1=	" << setprecision(8) << par[i][1] << "	+/-	" << par_err[i][1] << "	p0=	" << par[i][0] << "	+/-	" << par_err[i][0] << "	Chi2=	" << parChi[i] << "	ndf=	" << parNDF[i] << "	p-value=	" << p_value[i] << endl;
