...
void peakcali_sqrt_band_pxct_FWHM()//fit 12 X-ray/Gamma ray peaks' FWHM values measured by LEGe detector with sqrt function with a credible interval band
{
	...
	for (i = ID1; i <= ID2; i++) // no need to modify
	{
		...
		fwhm_func->GetParameters(par[i]);
		par_err[i][0] = fwhm_func->GetParError(0);
		par_err[i][1] = fwhm_func->GetParError(1);
		//par_err[i][2] = fwhm_func->GetParError(2);
		parChi[i] = fwhm_func->GetChisquare();
		parNDF[i] = fwhm_func->GetNDF();
		...
		{
			cout << "Eg=	" << energy_point[ipeak] << "	FWHM=	" << fwhm_func->Eval(energy_point[ipeak]) << "	err_FWHM=	" << err_point[ipeak] << endl;
		}
		...
		//		textpol1->AddText(paraprint);
		sprintf(paraprint, "Fano=%.9f+/-%.9f", par[i][1], par_err[i][1]);
		textpol1->AddText(paraprint);
		sprintf(paraprint, "Electronics=%.7f+/-%.7f", par[i][0], par_err[i][0]);
		textpol1->AddText(paraprint);
		...
		{
			residual[i][ii] = (fwhm_func->Eval(x_value[i][ii]) - y_value[i][ii]);
