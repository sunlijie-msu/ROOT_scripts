...
#include "TLegend.h"
using namespace std;
void peakfit_expdecay_band_lifetime_pxct_faketest_237Np() // gets htiming_lege_msd26_bin1ns from many Run3_timing_msdtotal_e_5358_5478_msdtotal_t.root files and fit exponential decay. Fit results are output to F:\e21010\pxct\peakpara.dat -> PXCT.xlsx
// Upstream code: Fill_histogram_random_decay_pxct_237Np.C
{
	double binwidth = 1;
	...
	int colors[5] = { kBlack, kRed, kAzure, kGreen, kMagenta };
	int colorsband[5] = { kGray + 1, kRed - 9, kAzure - 9, kGreen - 9, kMagenta - 9 };
	int Which_Dataset = 4; // Modify: 1 for MSDtotal; 2 for MSD26; 3 for MSDtotal; 4 for Fake test
	int Which_MSD;
	int Timing_resolution = 0;
	...
	if (Which_Dataset == 2)
	{
		...
		msd_e_cut_low = 5459; // for single peak fit
		msd_e_cut_high = 5499; // for single peak fit
	...
	for (i = 0; i <= 0; i ++)
	{
		...
		sprintf(filename, "%s%s", pathname, "Fake_decay_1e7.root"); // Fake test
		...
		sprintf(histo_name, "%s", "h_decay_exponential_Gaus3_TRandom3"); // Fake test
		...
		for (ii = 0; ii <= 0; ii++) // Not necessary modify: ii <=39, fit one histogram with many different fit start points = ii * 20 + bin_start_low
		{
			...
			histo[i]->GetXaxis()->SetRangeUser(-1500, 2000);
			...
			fEMG[ii]->SetParameters(1e7, 68, 0);//initial value
			fEMG[ii]->SetParLimits(0, 2e6, 9e7);//N
			fEMG[ii]->SetParLimits(1, 40, 140);//T
			fEMG[ii]->SetParLimits(2, -0.001, 0.001);//B
			...
			p_value[ii] = fEMG[ii]->GetProb();//This probability is not the “probability that your fit is good.” If you did many pseudo experiments (draw many random samples of data points from the assumed distribution (your fit function)), this is the percentage of experiments that would give χ2 values ≥ to the one you got in this experiment.
...
// 			outfile << histo_name << i << "	Constant*binsize" << ii << "=	" << constant[ii] << "	+/-	" << constant_err[ii] << "	Mean" << ii << "=	" << mean[ii] << "	+/-	" << mean_err[ii] << "	Maximum" << ii << "=	" << peakx[ii] << "	+/-	" << peakxerr[ii] << "	Sigma" << ii << "=	" << sig[ii] << "	+/-	" << sig_err[ii] << "	Tau" << ii << "=	" << tau[ii] << "	+/-	" << tau_err[ii] << "	A" << ii << "=	" << par[ii][0] << "	+/-	" << par_err[ii][0] << "	B" << ii << "=	" << par[ii][1] << "	+/-	" << par_err[ii][1] << "	Chi2" << ii << "=	" << parChi[ii] << "	NDF" << ii << "=	" << parNDF[ii] << "	Area" << ii << "=	" << par[ii][2] / binwidth << "	FWHM" << ii << "=	" << FWHM[ii] << "	+/-	" << FWHM_err[ii] << endl;
			...
			textgaus->AddText(paraprint);
			sprintf(paraprint, "Background%d=%.4f%s%.4f", ii, par[ii][2], "+/-", par_err[ii][2]);
			textgaus->AddText(paraprint);
			...
			sprintf(paraprint, "Time difference (ns)");
