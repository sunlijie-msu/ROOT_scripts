...
#include "TLegend.h"
using namespace std;
void peakfit_gausnerfcpol1_band_doublepeak_pxct() // get histogram and EMG fit two peaks
{
	...
	lower_search_bound[0][34] = 1080;	upper_search_bound[0][34] = 1092; //152Eu 1085 XtRa
	...
	sprintf(filename, "%s%s", pathname, "run0334_LEGe_241Am_Z7117_ChamberCenter_window1.5us_TrigRise0.064us_TrigGap0.952us_Th350_CFDDelay0.304us_Scale7_for_X_efficiency_cal.root");
	TFile* fin = new TFile(filename);//after this statement, you can use any ROOT command1 for this rootfile
	cout << filename << endl;

	for (i = ID1; i <= ID2; i++)//which detector no need to change
	{
		sprintf(histo_name, "%s", "hlege_e"); // modify
	...
	{
		for (ii = 8; ii <= 8; ii++)// modify which peak in one detector
		{
			...
			if (ii == 6) { gaplow = 1.4; gaphigh = 0.9; Eg = 40.1; }//152Eu 40.1 and 39.5 LEGe
			if (ii == 7) { gaplow = 0.7; gaphigh = 0.7; Eg = 33.196; }//241Am 33.2 LEGe
			if (ii == 8) { gaplow = 0.6; gaphigh = 0.6; Eg = 13.9; }//241Am 13.9 and 13.8 LEGe
			...
			if (ii == 25) { gaplow = 4.0; gaphigh = 6.0; Eg = 563.986; }//152Eu 564 566 XtRa double peak needed
			if (ii == 26) { gaplow = 6.0; gaphigh = 2.8; Eg = 586.2648; }//152Eu 586 XtRa double peak needed//not anymore due to room background 583 subtraction
			...
			if (ii == 34) { gaplow = 3.7; gaphigh = 6.6; Eg = 1085.837; } //152Eu 1085 XtRa
			...
			p2[ii] = new TF1("p2", "[0]*x+[1]-[0]*x-[1]+[2]/2/[3]*exp(0.5*([4]*[4]/([3]*[3]))+(x-[5])/[3])*ROOT::Math::erfc(1/sqrt(2)*([4]/[3]+(x-[5])/[4]))", histomin, histomax);//pure peak
			...
			if (ii == 26)
			{
				fEMG[ii]->SetParameters(aguess, bguess, peaky[ii], 0.2, sigmaguess, peakx[ii], peaky[ii], 0.04, sigmaguess, peakx[ii] - gaplow / 2);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ, [6]-N2, [7]-τ2, [8]-σ2, [9]-μ2. If the main peak is high energy one, the second peak is low energy one.
			}
			else
			{
				// Careful! The order of the parameters is important.
				fEMG[ii]->SetParameters(aguess, bguess, peaky[ii], 0.2, sigmaguess, peakx[ii], peaky[ii], 0.04, sigmaguess, peakx[ii] + gaplow / 2);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ, [6]-N2, [7]-τ2, [8]-σ2, [9]-μ2. If the main peak is low energy one, the second peak is high energy one.
			}			
			...
			//fEMG[ii]->SetParLimits(0,0,700);//Bkg A
			// 	fEMG[ii]->SetParLimits(1,-50000,300000);//Bkg B
			fEMG[ii]->SetParLimits(2, 1400, 1e5);//Constant,min,max
			fEMG[ii]->SetParLimits(3, 0.02, 0.05);//Tau
			fEMG[ii]->SetParLimits(4, 0.01, 0.2);//Sigma
			fEMG[ii]->SetParLimits(5, peakx[ii] - gaplow, peakx[ii] + gaphigh);//Mean
			fEMG[ii]->SetParLimits(5, 13.95, 14.06);//Mean
			fEMG[ii]->SetParLimits(6, 150, 1e4);//Constant_2nd,min,max
			fEMG[ii]->SetParLimits(7, 0.02, 0.05);//Tau_2nd
			fEMG[ii]->SetParLimits(8, 0.01, 0.2);//Sigma_2nd
			fEMG[ii]->SetParLimits(9, peakx[ii] + gaplow, peakx[ii] + gaplow);//Mean_2nd
			fEMG[ii]->SetParLimits(9, 13.79, 13.89);//Mean_2nd
			...
			topy = p2[ii]->GetMaximum(fitrange_min, fitrange_max);
			topx = p2[ii]->GetMaximumX(fitrange_min, fitrange_max);
			lower_half = p2[ii]->GetX(topy / 2.0, fitrange_min, topx, 1E-12);
			...
			textgaus->AddText(paraprint);
			sprintf(paraprint, "Res2nd%d=%.4f%%", ii, FWHM2nd[ii] / par[ii][9] * 100);
			textgaus->AddText(paraprint);
