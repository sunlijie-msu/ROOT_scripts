...
void peakfit_gausnerfcpol1_band_pxct() // get histogram and EMG fit one peak
{
	...
	lower_search_bound[0][35] = 15.5;	upper_search_bound[0][35] = 16.2; //241Am 15.9 LEGe
	...
	{
		for (ii = 4; ii <= 4; ii++)// modify which peak in one detector =0<=13
		{
			...
			if (ii == 35) { gaplow = 0.4; gaphigh = 0.38; Eg = 15.876; } //241Am 15.9 LEGe
			...
			fEMG[ii]->SetParLimits(2, 1e2, 3e6);//Constant,min,max
			fEMG[ii]->SetParLimits(3, 0.01, 0.1);//Tau
			fEMG[ii]->SetParLimits(4, 0.1, 0.25);//Sigma
