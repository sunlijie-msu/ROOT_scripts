...
void peakfit_gausnerfcpol1_band_pxct() // get histogram and EMG fit one peak
{
	...
	sprintf(filename, "%s%s", pathname, "run0008-00_LEGe_241Am_66min_cal.root"); // modify
...
// 			canvaspeak[i][ii]->SetBottomMargin(0.13);
			...
			histo[i]->GetYaxis()->SetTitle("Counts per 7 eV");//轴名
			...
			if (ii == 0) { gaplow = 0.5; gaphigh = 0.4; Eg = 5895; Eg2nd = 6490; }//55Fe 5.9 and 6.5
			if (ii == 2) { gaplow = 1.4; gaphigh = 1.4; Eg = 121.782; }//152Eu 122
			if (ii == 4) { gaplow = 1.0; gaphigh = 1.0; Eg = 59.541; }//241Am	59.5
			if (ii == 5) { gaplow = 0.8; gaphigh = 0.8; Eg = 26.345; }//241Am 26.3
 			if (ii == 7) { gaplow = 0.7; gaphigh = 0.7; Eg = 33.196; }//241Am 33.2
			if (ii == 9) { gaplow = 3.0; gaphigh = 3.0; Eg = 121.7817; }//152Eu 122 XtRa
			if (ii == 10) { gaplow = 3.0; gaphigh = 3.0; Eg = 344.2785; }//152Eu 344 XtRa
			if (ii == 11) { gaplow = 3.1; gaphigh = 3.1; Eg = 1299.1420; }//152Eu 1299 XtRa
			if (ii == 12) { gaplow = 33; gaphigh = 25; Eg = 5486; }//241Am 5486 MSD26
			...
			fEMG[ii]->SetParameters(aguess, bguess, peaky[ii], 0.04, sigmaguess, peakx[ii]);//initial value [0]-A, [1]-B, [2]-N, [3]-τ, [4]-σ, [5]-μ
			// 			fEMG[ii]->SetParLimits(0,-500,500);//Bkg A
			// 			fEMG[ii]->SetParLimits(1,-50000,300000);//Bkg B
			fEMG[ii]->SetParLimits(2, 60, 120);//Constant,min,max
			fEMG[ii]->SetParLimits(3, 0.001, 0.05);//Tau
			fEMG[ii]->SetParLimits(4, 0.01, 0.2);//Sigma
			fEMG[ii]->SetParLimits(5, peakx[ii] - gaplow/2, peakx[ii] + gaphigh/2);//Mean
			fEMG[ii]->SetParLimits(5, 11.9, 12.1);//Mean
...
// 			if (inflation_factor < 1)
			...
			Double_t top, lower_half, higher_half;
			top = p[ii]->GetMaximum(fitrange_min, fitrange_max);
			lower_half = p[ii]->GetX(top/2.0, fitrange_min, peakx[ii], 1E-12);
			higher_half = p[ii]->GetX(top / 2.0, peakx[ii], fitrange_max, 1E-12);
