...
#include "TLegend.h"
using namespace std;
void peakfit_gausnerfcpol1_band_timing_pxct() // get histogram and EMG fit one peak
{
	...
	int fit_Nbins;
	const int peaknum = 30;//search peak numbers
	...
	sprintf(filename, "%s%s", pathname, "run0079_80_LEGe_MSD_241Am_inChamber_window1.5us_CFDdelay_adjusted_3840min_cal.root"); // modify
	...
	{
		for (ii = 23; ii <= 23; ii++)// modify which peak in one detector =0<=13
		{
			...
			histo[i]->SetTitle("");//图名
			histo[i]->GetXaxis()->SetTitle("Time difference LEGe - MSD26 (ns)");//轴名
			...
			if (ii == 23) { gaplow = 50; gaphigh = 1400; Eg = 59.541; }//241Am Timing MSD-LEGe
			...
			fEMG[ii]->SetParLimits(2, 190000, 300000);//Constant,min,max
			...
			histo[i]->SetLineColor(kViolet+1);
			histo[i]->SetMarkerColor(kViolet+1);
...
// 			if (inflation_factor < 1)
			...
			graph_residual[i]->SetLineColor(kViolet+1);
			graph_residual[i]->SetMarkerStyle(6);
			graph_residual[i]->SetMarkerColor(kViolet+1);
			canvaspeak[i][ii]->cd();//进入画布
			pad2->Draw();
			pad2->cd();
