...
#include "TLegend.h"
using namespace std;
void txttoh1()//read data from a txt file and copy histogram from a root file and file histos in a new root file.
{// input bin by bin, not event by event
	...
	char rawrootname[80];
	TCanvas *canvas;
	int Entries=30;//modify
	ifstream infile("C:/Si24/Si22peakcali/Xu/inputforh1.dat",ios::in);
	sprintf(rootname,"%s","C:/Si24/Si22peakcali/Xu/outputforh1.root");
//	sprintf(rawrootname,"C:/Si24/Si22peakcali/Xu/S27_pz_new.root");//modify if change path or nuclide
...
//	cout<<rawrootname<<endl;
	...
	TH1F *h1 = new TH1F("DSL1data","DSL1data",30,7610,7910);//create a histogram
...
// 	}
	canvas->cd();//进入画布
	h1->GetXaxis()->SetTitle("Energy (keV)");//轴名
	h1->GetYaxis()->SetTitle("Counts per 10 keV");//轴名
